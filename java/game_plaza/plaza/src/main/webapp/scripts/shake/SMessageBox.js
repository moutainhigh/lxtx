...
function noticeBoxEx(content1, color1, content2, color2){
	...
	layer_notice_box.x = convertToScreenCenterX(imgSize[IMG_NOTICE_BOX].w);
	layer_notice_box.y = convertToScreenCenterY(imgSize[IMG_NOTICE_BOX].h);
	layer_notice_box.addEventListener(LEvent.ENTER_FRAME,onNoticeBoxFrame_Ex);
...
var max_message_box_frame_count = 150;
...
var max_message_box_frame_count_Ex = 210;
function onNoticeBoxFrame_Ex(event){
    if (++notice_frame_count > max_message_box_frame_count_Ex){
        layer_notice_box.remove();
        layer_notice_box = null;
    }
}
