 ...
 * Class SecurePay
 */
class SecurePay extends base
{
    var $code, $title, $description, $enabled, $trade_no, $form_action_url, $paymentZoneWidth, $paymentZoneCustomWidth, $paymentZoneMobileWidth, $securePayConfig;
    /**
    ...
    function SecurePay()
    {
        global $order;
        $this->code = 'securepay';
        $this->title = "SecurePay";
        $this->description = "";
        $this->sort_order = MODULE_PAYMENT_SECUREPAY_SORT_ORDER;
        $this->enabled = ((MODULE_PAYMENT_SECUREPAY_STATUS == 'True') ? true : false);
        $this->paymentZoneCustomWidth = MODULE_PAYMENT_SECUREPAY_ZONE_CUSTOM_WIDTH == '' ? "310px" : MODULE_PAYMENT_SECUREPAY_ZONE_CUSTOM_WIDTH;
        $this->paymentZoneWidth = (MODULE_PAYMENT_SECUREPAY_ZONE_WIDTH == 'custom') ? MODULE_PAYMENT_SECUREPAY_ZONE_CUSTOM_WIDTH : MODULE_PAYMENT_SECUREPAY_ZONE_WIDTH;
        $this->paymentZoneMobileWidth = (MODULE_PAYMENT_SECUREPAY_ZONE_MOBILE_WIDTH == '') ? "80%" : MODULE_PAYMENT_SECUREPAY_ZONE_MOBILE_WIDTH;
        ...
        $this->securePayConfig = $this->getSecurePayConfig();
    ...
    function update_status()
    {
        ...
        if (($this->enabled == true) && ((int)MODULE_PAYMENT_SECUREPAY_ZONE > 0)) {
            $check_flag = false;
            $check_query = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_SECUREPAY_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function javascript_validation()
    {
        ...
        $js .= '  if (payment_value == "' . $this->code . '") {' . "\n" .
            '    var SECUREPAY_number = document.checkout_payment.ekData1.value;' . "\n" .
            '    var SECUREPAY_cvv = document.checkout_payment.ekData2.value;' . "\n" .
            '    var SECUREPAY_expires_month = document.checkout_payment.ekData3.value;' . "\n" .
            '    var SECUREPAY_expires_year = document.checkout_payment.ekData4.value;' . "\n";

        $js .= '    if (SECUREPAY_number.length != 16) {' . "\n" .
            '      error_message = error_message + "' . $this->securePayConfig['messages']['cartNoError'] . '";' . "\n" .
            '      error = 1;' . "\n" .
            '    }' . "\n";
        $js .= '    if (SECUREPAY_cvv.length != 3 && SECUREPAY_cvv.length != 4) {' . "\n" .
            '      error_message = error_message + "' . $this->securePayConfig['messages']['cvvError'] . '";' . "\n" .
            '      error = 1;' . "\n" .
            '    }' . "\n";
        $js .= '    if (SECUREPAY_expires_month =="") {' . "\n" .
            '      error_message = error_message + "' . $this->securePayConfig['messages']['monthError'] . '";' . "\n" .
            '      error = 1;' . "\n" .
            '    }' . "\n";
        $js .= '    if (SECUREPAY_expires_year =="") {' . "\n" .
            '      error_message = error_message + "' . $this->securePayConfig['messages']['yearError'] . '";' . "\n" .
            '      error = 1;' . "\n" .
            '    }' . "\n";
        ...
        /*        $js .= '    if(!error){' . "\n" .
                    ...
                    '        } else { ' . "\n" .
                    '         alert("' . MODULE_PAYMENT_SECUREPAY_TEXT_JS_GLEEPAY_RESUBMIT . '"); ' . "\n" .
                    '         return false; ' . "\n" .
                    '        } ' . "\n" .
                    '    } ' . "\n";*/
    ...
    function selection()
    {
        global $insert_id, $db, $messageStack, $order_total_modules, $order, $securePayConfig;

        $expires_month[] = array(
            "id" => "",
            "text" => $this->securePayConfig['messages']['month']
        );
        $expires_year[] = array(
            "id" => "",
            "text" => $this->securePayConfig['messages']['year']
        );
        for ($i = 1; $i < 13; $i++) {
            $expires_month[] = array(
                'id' => sprintf('%02d', $i),
        ...
        $monitorUrl = $this->securePayConfig  ["monitorUrl"] . '?CartID=' . $_SESSION['cart']->cartID . '&AcctNo=' . MODULE_PAYMENT_SECUREPAY_MID . '&Framework=ZenCart&IVersion=V7.0&CMSVersion=' . $this->zencartVersion . '&PHPVersion=' . phpversion();
        ...
        $payZoneWidth = $IVersion == 'mobile' ? $this->paymentZoneMobileWidth : $this->paymentZoneWidth;

        $expiresSelectWidth = "80px";
        if ($payZoneWidth < 180) {
            $expiresSelectWidth = "70px";
        }
        if (strstr($payZoneWidth, '%')) {
        ...
        $selection = array(
            'id' => $this->code,
            'module' => zen_image('images/securepay/vmjH40.png', 'Credit Card Payment Online') .
                '</label>'
                . '<style>
            ...
            'fields' => array(
                ...
                array(
                    'title' => $this->securePayConfig['messages']['cardNumber'] . '<span style="color:red;display:inline-block;" id="">*</span>',
                    'field' => zen_draw_input_field('ekData1', ''
                        , 'id="' . $this->code . '-cardNo" size="20px" maxlength="16" onkeyup="this.value=this.value.replace(/\D/g,\'\');checkCardType(this);"  style="border: 1px solid #BBBBBB;float: left;margin: 5px 10px 1em 2em;height:20px;font: 12px/20px Verdana;color: #666666;background-image:url(images/securepay/vmj.png);background-position:right center;background-repeat:no-repeat;width:' . $payZoneWidth . ';" '
                        , $this->isSupportHtml5() ? "tel" : "text"),
                    'tag' => $this->code . '-cardNo'
                ),
                //输出有效期下拉框
                array(
                    'title' => $this->securePayConfig['messages']['expirationDate'] . '<span style="color:red;display:inline-block;">*</span>',
                ...
                //输出安全码
                array(
                    'title' => $this->securePayConfig['messages']['cvv'] . '<span style="color:red;display:inline-block;">*</span>',
                    'field' => zen_draw_password_field('ekData2', '', 'id="' . $this->code . '-cvv" size="8px" maxlength="4" style="box-sizing: border-box;border: 1px solid #BBBBBB;float: left;margin: 5px 10px 1em 2em;height:20px;font: 12px/20px Verdana;color: #666666;width:' . $expiresSelectWidth . ';"')
                        . '<small style="position: relative;float: left;margin-top: 4px;margin-left: 5px;line-height:20px;">'
                        . zen_image("./images/securepay/cartpic.jpg", $this->securePayConfig['messages']['cvvNote'], 51, 30, "style='margin-left:-10px;'") . '</small>',
                    'tag' => $this->code . '-cvv'
                ),
                array(
                    'field' => '<img src="images/securepay/certservices.png" style="padding: 20px 30px 20px;width:' . $payZoneWidth . '"/>' .
                        '<script language="javascript" type="text/javascript" src="includes/modules/payment/securepay.js"></script>'
                )
            )
        );
        return $selection;
    ...
    function pre_confirmation_check()
    {
        ...
        $_SESSION['issuing_bank'] = $_POST['ekData5'];
        return false;
    }
    ...
    function create_order()
    {
        ...
        $_SESSION['ekaepay_order_id'] = $order->create($order_totals, 2);
        $order->create_add_products($_SESSION['ekaepay_order_id']);


        /*try{
            $order->send_order_email($_SESSION['ekaepay_order_id'],2);
        }catch(Exception $e){
    ...
    {
        $url_server = $this->securePayConfig["gatewayUrl"];
    ...
    function before_process()
    {
        ...
        if (empty($_SESSION['cardNo'])) {
            ...
            //下订单
            $this->confirmation("ekaepay_ok");
            $insert_id = $_SESSION['ekaepay_order_id'];
            ...
            if ($result == null || !is_array($result)) {
                $messageStack->add_session('shopping_cart', $this->securePayConfig["messages"]["errorNote"], 'error');
            ...
            if (zen_not_null($result) && $result["status"] != '0000') {
                if ($result["isPendingPayment"] == true) {
                    $messageStack->add_session('shopping_cart', $this->securePayConfig["messages"]["payPending"], 'error');
                } else {
                    $messageStack->add_session('shopping_cart', str_replace("@@@", $result["msg"], $this->securePayConfig["messages"]["payFailure"]), 'error');
                }
                zen_redirect(zen_href_link(FILENAME_SHOPPING_CART, '', 'SSL', true, false));
                exit;
            }
            ...
            $MD5key = trim(MODULE_PAYMENT_SECUREPAY_MD5KEY);
...
//回调执行此方法
    function notify($response)
    {
        ...
        if ($HashValue == $md5sign) {
            $redirect = FILENAME_SHOPPING_CART;//FILENAME_CHECKOUT_PAYMENT;
            //清除session
            //返回码判断
            $notify = 0;
			if ($RespCode == '0000') {
    ...
    function update_order_status($order_id, $status, $transactionid, $notify = 0)
    {
        ...
        if (($check_status->fields['orders_status'] != $status_id)) {
            ...
            $db->Execute("insert into " . TABLE_ORDERS_STATUS_HISTORY . "
                      ...
                      now(),
                      '" . zen_db_input($notify) . "',
                      'Pay notice [Ekaepay TradeNo: {$transactionid}]')");
        }
    }
    ...
    function buildNameValueList($orders_id)
    {
        ...
        $BillNo = $orders_id;
        $MD5key = trim(MODULE_PAYMENT_SECUREPAY_MD5KEY);
        $MerNo = trim(MODULE_PAYMENT_SECUREPAY_MID);
        ...
        $post_data = array(
            ...
            'IVersion' => "V7.0",
            'IFrame' => 1,
            'cardpay_brower' => $BroserType,
            'cardpay_brower_lang' => $BrowserLang,
            'cookies' => $cookies,
            'Language' => $LangCode,
    ...
    function check()
    {
        ...
        if (!isset($this->_check)) {
            $check_query = $db->Execute("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_SECUREPAY_STATUS'");
    ...
    function install()
    {
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Open payment mode', 'MODULE_PAYMENT_SECUREPAY_STATUS', 'True', 'If you would like to open payment mode', '6', '1', 'zen_cfg_select_option(array(\'True\', \'False\'), ', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('SecurePay Id', 'MODULE_PAYMENT_SECUREPAY_MID', '', '', '6', '2', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('SecurePay Key', 'MODULE_PAYMENT_SECUREPAY_MD5KEY', '', '', '6', '3', now())");
        //支付区域宽度
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Payment Zone Width', 'MODULE_PAYMENT_SECUREPAY_ZONE_WIDTH', '1', ' Please choose the width of payment zone', '6', '4','','zen_draw_pull_down_menu(\"configuration[MODULE_PAYMENT_SECUREPAY_ZONE_WIDTH]\",array(array(id=>\"310px\",text=>\"310px\"),array(id=>\"240px\",text=>\"240px\"),array(id=>\"180px\",text=>\"180px\"),array(id=>\"custom\",text=>\"Custom Width\")),', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Payment Zone Custom Width', 'MODULE_PAYMENT_SECUREPAY_ZONE_CUSTOM_WIDTH', '', 'If you select Custom width for Payment Zone Width,you must input this.', '6', '5', now())");
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Payment Zone Mobile Width', 'MODULE_PAYMENT_SECUREPAY_ZONE_MOBILE_WIDTH', '80%', 'Please input the width of payment zone for mobile.', '6', '6', now())");

        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Payment Zone', 'MODULE_PAYMENT_SECUREPAY_ZONE', '0', 'Unchoose is all area can use payment method', '6', '14', 'zen_get_zone_class_title', 'zen_cfg_pull_down_zone_classes(', now())");
        // 订单初始状态
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Order Orginal Status', 'MODULE_PAYMENT_SECUREPAY_ORDER_STATUS_ID', '1', 'Please choose pending', '6', '12', 'zen_cfg_pull_down_order_statuses(', 'zen_get_order_status_name', now())"); //***** modified.*****
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Display Sequence', 'MODULE_PAYMENT_SECUREPAY_SORT_ORDER', '1', 'The Sequence of payment method,the lower rank first.', '6', '6', now())");
    ...
    {
        return array(
            'MODULE_PAYMENT_SECUREPAY_STATUS',
            'MODULE_PAYMENT_SECUREPAY_MID',
            'MODULE_PAYMENT_SECUREPAY_MD5KEY',
            'MODULE_PAYMENT_SECUREPAY_ZONE',
            'MODULE_PAYMENT_SECUREPAY_ZONE_WIDTH',
            'MODULE_PAYMENT_SECUREPAY_ZONE_CUSTOM_WIDTH',
            'MODULE_PAYMENT_SECUREPAY_ZONE_MOBILE_WIDTH',
            'MODULE_PAYMENT_SECUREPAY_ORDER_STATUS_ID',
            'MODULE_PAYMENT_SECUREPAY_SORT_ORDER'
        );
    }
    ...
    function getSecurePayConfig()
    {
        ...
        if (!isset($_SESSION['securePayConfig'])) {
            ...
            if (strtoupper(CHARSET) == 'ISO-8859-1') {
                foreach ($securePayConfig as $key => $value) {
                    if (is_array($value)) {
                        foreach ($value as $key_s => $value_s) {
                            $securePayConfig[$key][$key_s] = utf8_decode($value_s);
                        }
                    } else {
                        $securePayConfig[$key] = utf8_decode($value);
                    }
                }
            }
