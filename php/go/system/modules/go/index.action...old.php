...
class index extends base {
	...
	public function init(){		
		...
		//其他商品
		$shoplistqita=$this->db->GetList("select * from `@#_shoplist` where `cateid`='5' and `q_uid` is null ORDER BY `id` ASC LIMIT 10");
		//最新上架
		...
		if(!$item)_message("没有这个商品！",WEB_PATH,3);
			...
			if($go_record_list){
				...
				foreach($go_record_list as $user){
					...
					$html.='<p ><span><a href="'.WEB_PATH.'/uname/'.idjia($user['uid']).'" target="_blank" class="blue">'._strcut($user['username'],26).'</a>('.get_user_ip($user['uid'],'').')云购了<em>'.$user['gonumber'].'</em>人次</span></p></div></div>';
				}
