...
if ($check_grid) {
  ...
  $attrib_grid = '<table id="attrib-grid-table">'."\n" . 
	...
	if ($grv == 0) {
	  $attrib_grid .= '  <td rowspan="'.$grv_size.'" id="attrib-grid-opt-nameV">'.$grid_records['V']['name'].(zen_not_null($grid_records['H']['comment']) ? '<br /><span id="attrib-grid-opt-commentV">'.$grid_records['V']['comment'].'</span>' : '').'</td>'.'</tr>'."\n";
	}
                    ...
                    if ($_POST['id']) {
                      ...
                      if ( $products_options_names->fields['products_options_rows'] > 1 ) {
                        ...
                        //                $tmp_html .= '  <input type="reset">';
					  	...
					  	if ($products_options_names->fields['products_options_name'] == 'Your Name') { $options_extra = ' reg="^\w{1,14}$"'; } 
