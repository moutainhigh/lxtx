...
//商品分类列表
	public function goods_type(){
			...
			include $this->tpl(ROUTE_M,'shop.types');
	}



//商品分类列表
	public function channnel_type(){

			include $this->tpl(ROUTE_M,'channel.types');
	}
...
//商品分类删除
	public function type_del(){
		...
		$q1 = array();
		$q1 = $this->db->GetList("SELECT sgid FROM `@#_channel`");
	
		if(!empty($q1)){
			foreach ($q1 as $v) {
				if(!empty($v['sgid'])){
					$where = '(' . $v['sgid'] . ')';
					$q3 = $this->db->GetList("SELECT * FROM `@#_type` WHERE `id` in " . $where);
					if(!empty($q3)){
						_message("请先删除渠道对应的分类");
						include $this->tpl(ROUTE_M,'shop.types');
						exit();
					}
				}	
		...
		$q2 = $this->db->Query("DELETE FROM `@#_type` WHERE `id` = '$id' LIMIT 1");	
...
//渠道删除
	public function channel_del(){
		...
		$this->db->Autocommit_start();	
			...
			$q2 = $this->db->Query("DELETE FROM `@#_channel` WHERE `id` = '$id' LIMIT 1");	
				$this->db->Autocommit_commit();
...
if ( strlen($post_clearassign) > 0 ) {
		  $query_clear = $this->db->Query("UPDATE `@#_goods_item` SET 
                        `zhiding` = NULL
                         where `id` = '$gid'");
		  if ( $query_clear ) {
                    $this->db->Autocommit_commit();
		  ...
		  $query_assign = $this->db->Query("UPDATE `@#_goods_item` SET 
			`zhiding` = '$post_uid' 
	        where `id` = '$gid'");
	...
	public function goods_edit(){
		...
		generate_log("title, id:".$shopinfo['title']."  ".$shopinfo['id']);
		...
		if(isset($_POST['dosubmit'])){		
			...
			//update the common attributes of a specific goods
			$sql="UPDATE `@#_goods` SET `cateid` = '$cateid',
			...
			if($this->db->Query($sql)){			
				$this->db->Query("UPDATE `@#_goods_item` set `renqi` = '$goods_key_renqi', `xsjx_time` = '$xsjx_time', `pos` = '$goods_key_pos' where `id` = '$shopinfo[id]'");
				$this->db->Autocommit_commit();	
				_message("修改成功!");
			}else{	
				generate_log($sql);
				generate_log("failed in edit goods.");
				$this->db->Autocommit_rollback();
				_message("修改失败!");
		...
		generate_log('cateinfo:'.$cateinfo['name']."  ".$cateinfo['cateid']);
...
//查表看看是否有重复
			 ...
			 $cag=$this->db->GetList("SELECT * FROM `@#_type`" );

foreach ($cag as $k => $v) {
	if ($v['name']==$name || $v['id']==$id) {


		_message("id重复或者分类名称重复");
		 include $this->tpl(ROUTE_M,'type.add');die;
	}
}

			$describe =($_POST['describe']);// 添加分类
			
			$this->db->Autocommit_start();

					
			$query_1 = $this->db->Query("INSERT INTO `@#_type` (`id`,`name`, `describe`) VALUES ('$id','$name','$describe')");			
			 ...
			 include $this->tpl(ROUTE_M,'shop.types');
			  ...
			  include $this->tpl(ROUTE_M,'type.add');
		}
...
//查表看看是否有重复
			 $cag=$this->db->GetList("SELECT * FROM `@#_channel`" );
		...
		_message("id重复或者分类名称重复");
		 include $this->tpl(ROUTE_M,'channel.add');die;
	}
}
			...
			$query_1 = $this->db->Query("INSERT INTO `@#_channel` (`id`,`name`, `channel`, `sgid`, `pid`, `status`) VALUES ('$id','$name','$channel','$sgid','$pid','$status')");			
			 ...
			 include $this->tpl(ROUTE_M,'channel.types');
			  ...
			  include $this->tpl(ROUTE_M,'channel.add');
		}
...
//查表看看是否有重复
			...
			$query_1 = $this->db->Query("UPDATE `@#_type` SET `describe` = '$describe',
		...
		$this->db->Autocommit_start();	
			 ...
			 $c=$this->db->GetOne("SELECT * FROM `@#_type` where id = $id ");	
			$date = $this->db->Autocommit_commit();


			  include $this->tpl(ROUTE_M,'type.edit');
		}
...
//编辑商品
	public function channel_edit(){
		...
		if(isset($_POST['dosubmit'])){		
			...
			$query_1 = $this->db->Query("UPDATE `@#_channel` SET `channel` = '$channel',
		...
		$this->db->Autocommit_start();	
			 ...
			 $c=$this->db->GetOne("SELECT * FROM `@#_channel` where id = $id ");	
			$date = $this->db->Autocommit_commit();


			  include $this->tpl(ROUTE_M,'channel.edit');
		}
	...
	public function goods_add(){		
		...
		if(isset($_POST['dosubmit'])){		
			...
			$canyurenshu = 0;	
			if($_POST['dosubmit']=='piliang'){
				$_POST['goods_key'] = json_decode(stripslashes($_POST['goods_key']),true);
			}	
			...
			if(isset($_POST['uppicarr'])){
				if($_POST['dosubmit']=='piliang'){
					$_POST['uppicarr'] = json_decode(stripslashes($_POST['uppicarr']),true);
				}
				$picarr = serialize($_POST['uppicarr']);
...
//			$query_1 = $this->db->Query("INSERT INTO `@#_shoplist` (`cateid`, `sgid`, `brandid`, `title`, `title_style`, `title2`, `keywords`, `description`, `money`, `yunjiage`, `zongrenshu`, `canyurenshu`,`shenyurenshu`, `qishu`,`maxqishu`,`thumb`, `picarr`, `content`,`xsjx_time`,`renqi`,`pos`, `time`) VALUES ('$cateid','$sgid', '$brandid', '$title', '$title_style', '$title2', '$keywords', '$description', '$money', '$yunjiage', '$zongrenshu', '$canyurenshu','$shenyurenshu', '1','$maxqishu', '$thumb', '$picarr', '$content','$xsjx_time','$goods_key_renqi', '$goods_key_pos','$time')");			

			//add a new goods means two steps: 1. create a record in go_goods, 2. create a record in go_goods_item
			$query_1 = $this->db->Query("INSERT INTO `@#_goods` (`cateid`, `sgid`, `brandid`, `title`, `title_style`, `title2`, `keywords`, `description`, `money`, `yunjiage`, `zongrenshu`,`maxqishu`,`thumb`, `picarr`, `content`,) VALUES ('$cateid','$sgid', '$brandid', '$title', '$title_style', '$title2', '$keywords', '$description', '$money', '$yunjiage', '$zongrenshu', '$maxqishu', '$thumb', '$picarr', '$content')");					
			$shopid = $this->db->insert_id();

			//step2, create the goods item
			$query_goods_item = $this->db->Query("INSERT INTO `@#_goods_item` (`sid`, `zongrenshu`, `shenyurenshu`, `qishu`, `maxqishu`, `codes_table`, `xsjx_time`, `renqi`, `pos`, `time`) VALUES ('$shopid', '$zongrenshu', '1', '$maxqishu', '', '$xsjx_time','$goods_key_renqi', '$goods_key_pos','$time')");
			$goodsItemId = $this->db->insert_id();
			...
			$query_2 = content_get_go_codes($zongrenshu,3000,$goodsItemId);

			$query_3 = $this->db->Query("UPDATE `@#_goods_item` SET `codes_table` = '$query_table', `def_renshu` = '$canyurenshu' where `id` = '$goodsItemId'");

			if($query_1 && $query_goods_item  && $query_2 && $query_3){
	...
	public function goods_del(){
		...
		$q2 = $this->db->Query("DELETE FROM `@#_goods_item` WHERE `id` = '$shopid' LIMIT 1");	
	...
	public function goods_del_key(){
		...
		if($key=='yes'){			
			...
			$shoplist = $this->db->GetOne("select * from `@#_shoplist_del` where `id` = '$shopid' LIMIT 1");
			//insert this record into go_goods_item
			$q1 = $this->db->Query("INSERT INTO `@#_goods_item` select `id`, `sid`, `zongrenshu`, `canyurenshu`, `shenyurenshu`, `qishu`, `maxqishu`, `codes_table`, `xsjx_time`, `pos`, `renqi`, `time`, `order`, `q_uid`, `q_user`, `q_user_code`, `q_content`, `q_counttime`, `q_end_time`, `q_showtime`, `zhiding`, `q_status` from `@#_shoplist_del` where `id` = '$shopid' LIMIT 1 ");
	...
	public function goods_listorder(){		
		...
		if($this->segment(4)=='dosubmit'){
			...
			foreach($_POST['listorders'] as $id => $listorder){
				...
				$this->db->Query("UPDATE `@#_goods_item` SET `order` = '$listorder' where `id` = '$id'");		
	...
	*/	
	...
	public function goods_max_qishu(){
		...
		$info = $this->db->GetOne("SELECT * FROM `@#_goods_item` where `id` = '$gid' and `q_end_time` is not null");
		...
		$ret = $this->db->Query("UPDATE `@#_goods_item` SET `maxqishu` = '$qishu' where `sid` = '$info[sid]'");
		...
		if(isset($_POST['money']) || isset($_POST['yunjiage'])){
			...
			if(!empty($lists)){
				foreach ($lists as $k => $v) {
					...
					$this->db->Query("INSERT INTO  `@#_member_account` SET `uid` = $uid, `type`=1, `pay`='账户', `content`='$pay_type', `money`='$money',`time`=$time, `add_time`=$time");
				}
			}
