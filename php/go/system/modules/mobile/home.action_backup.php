...
public function invite(){
	...
	public function getnewtasks() {
		...
		$tasklist = $this->db->GetList("select * from `@#_goods_card_task` where uid = $uid and (status = 0 or status = 1) order by id desc");
	...
	public function convertgamebalance() {
		...
		$balance = $uinfo['money'];
		$suggested = 50 * (int)(intval($balance)/50);
		...
		if ($_POST['submit1']) {
			...
			if ($amount % 50 > 0) {
				_messagemobile("输入的云购币数量应该是50的整数倍",WEB_PATH."/mobile/home/convertgamebalance");
			}

			if ($amount <= 0) {
				_messagemobile("云购币数量至少为50枚",WEB_PATH."/mobile/home/convertgamebalance");	
			}
	...
	public function exchangeconfirm() {
		...
		/*$phoneend = substr($member['mobile'], 7);

		if ($_POST['submit1']) {
			$exchangemethod = $_POST['exchangemethod'];
			if ($exchangemethod == 'mobilerecharge') {
				
			} else if ($exchangemethod == 'alipay') {
				header("http://jishou.sino518.com/sale/exchange/apply?mobile=$mobile&day=$day&token=$token");
				exit;
			}
		}

		include templates("mobile/user","exchangeconfirm");*/
	}
