...
class wapwx
{
	...
	{	
		...
		<form action="'.$actionpath.'" method="post" id="orderForm">
			...
			<input type="hidden" name="payType" id="_payType" value="wxgzh"/>
		...
		window.onload = function(){
			try{
			 	if(window.apk && window.apk.wx && typeof(window.apk.wx) == "function"){
			 		window.apk.wx("' . $this->config["code"] . '", ' . $this->config['money'] . ');
				}  else {
					$("#orderForm").submit();
				}
			}catch(e){
