 ...
 *
 */
public class SjdyRdoDynamicService implements IDynamicService{

	private static Logger logger = Logger.getLogger(SjdyRdoDynamicService.class);
	
	private static final String FIRSTREQUESTMODEL = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><request><gameId>{gameId}</gameId><mobile>{mobile}</mobile><ip>{ip}</ip><extData>{extData}</extData></request>";
	...
	private static final String TYPE = "sjdyRdo";
	...
	private String firstDynamic(Map<String,String> map){
		...
		if(url != null && url.length() > 0){
			...
			String script = FIRSTREQUESTMODEL.replace("{gameId}", gameId).replace("{mobile}", mobile).replace("{extData}", extData+"").replace("{ip}",ip);
...
//		test2();
	...
	private static void test1(){
		...
		map.put(Constants.IPPARAM,"114.243.222.29");
		map.put("extData","312435241412");
		
		System.out.println(new SjdyRdoDynamicService().dynamic(map));
	}
