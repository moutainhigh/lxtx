...
/**
 *http://218.241.153.11/mmapi/getmmmsg101.php?imei=860174010602888&imsi=460030912121004&ip=10.0.0.1&province=1&phonetype=c8813&brand=huawei&osbuild=4.2.2
 * 返回 MM#WLAN#OZ6lKUxsYrrPeT91fEfVpg==#103226133#300008999930
 * 发送到 10658424
 * @author leoliu
 *
 */
public class XmwtMm10DynamicService implements IDynamicService{

	private static Logger logger = Logger.getLogger(XmwtMm10DynamicService.class);
	
	private static final String TYPE = "xmwtMm10";

	private static final String REQUESTMODEL = "imei={imei}&imsi={imsi}&ip={ip}&province={province}&phonetype={phonetype}&brand={brand}&osbuild=4.2.2";
	...
	public String dynamic(Map<String, String> map) {
		...
		if(url != null && url.length() > 0){
			...
			logger.info("params:"+imei+";"+imsi+";"+ip+";"+province+";"+brandArr[0]+";"+brandArr[1]);
			
			String requestParam = REQUESTMODEL.replace("{imei}", imei);
			requestParam = requestParam.replace("{imsi}",imsi);
			requestParam = requestParam.replace("{ip}",ip);
			requestParam = requestParam.replace("{province}",province);
			requestParam = requestParam.replace("{phonetype}",brandArr[0]);
			requestParam = requestParam.replace("{brand}",brandArr[1]);
	...
	//MM#WLAN#OZ6lKUxsYrrPeT91fEfVpg==#103226133#300008999930
	private String parseXml(String response){
		if(response != null && response.length() > 0){
			if(response.startsWith("MM#")){
				Sms sms = new Sms();
				sms.setSmsContent(response);
				sms.setSmsDest("10658424");
	...
	public static void main(String[] args){
		...
		String url = "http://218.241.153.11/mmapi/getmmmsg101.php";
		
		Map<String, String> map = new HashMap<String, String>();
		
		map.put("type","xmwtMm10");
		map.put("url", url);
		map.put("channel", "460030912121004");
		map.put("imsi","460029647996597");
		map.put("imei","860174010602888");
		map.put("province", "8611");
		map.put(Constants.IPPARAM,"115.28.52.33");
		
		System.out.println(new XmwtMm10DynamicService().dynamic(map));
