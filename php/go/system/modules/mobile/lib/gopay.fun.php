...
function gopay($member,$shopu,$shoprc,$shopid){
	...
	if($member['money']>=$shopu){
		...
		for($i=1;$i<$countid;$i++){
			...
			$mysql_model->Query("UPDATE `@#_goods_item` SET canyurenshu='".$canyurenshu."' where id='".$shopid[$i]."'");
...
function tencord($time,$num,$shopid,$qishu,$shoptitle,$shopmoney,$shopjiage){
	...
	$qishu=$qishu+1;
	$mysql_model->Query("UPDATE `@#_goods_item` SET qishu='$qishu',canyurenshu='0' where id='$shopid'");			
