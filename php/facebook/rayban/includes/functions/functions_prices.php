    ...
    if (SHOW_SALE_DISCOUNT_STATUS == '1' and ($display_special_price != 0 or $display_sale_price != 0)) {
      if ($display_sale_price) {
        if (SHOW_SALE_DISCOUNT == 1) {
          ...
          $show_sale_discount = '<p><span class="productPriceDiscount">' . PRODUCT_PRICE_DISCOUNT_PREFIX . $show_discount_amount . PRODUCT_PRICE_DISCOUNT_PERCENTAGE . '. available in 2 days.</span></p>';

        } else {
          $show_sale_discount = '<span class="productPriceDiscount">' . PRODUCT_PRICE_DISCOUNT_PREFIX . $currencies->display_price(($display_normal_price - $display_sale_price), zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . PRODUCT_PRICE_DISCOUNT_AMOUNT . '. available in 2 days.</span>';
        }
      } else {
        if (SHOW_SALE_DISCOUNT == 1) {
          $show_sale_discount = '<span class="productPriceDiscount">'  . PRODUCT_PRICE_DISCOUNT_PREFIX . number_format(100 - (($display_special_price / $display_normal_price) * 100),SHOW_SALE_DISCOUNT_DECIMALS) . PRODUCT_PRICE_DISCOUNT_PERCENTAGE . '. available in 2 days.</span>';
        } else {
          $show_sale_discount = '<span class="productPriceDiscount">' . PRODUCT_PRICE_DISCOUNT_PREFIX . $currencies->display_price(($display_normal_price - $display_special_price), zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . PRODUCT_PRICE_DISCOUNT_AMOUNT . '. available in 2 days.</span>';
        }
      }
    }

    if ($display_special_price) {
      $show_normal_price = '<p class="old-price">

<span id="old-price-327" class="price">' . $currencies->display_price($display_normal_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . ' </span></p>';
      if ($display_sale_price && $display_sale_price != $display_special_price) {
        ...
        if ($product_check->fields['product_is_free'] == '1') {
          ...
          $show_special_price = '&nbsp;' . '<p class="special-price">

<span id="product-price-327" class="price">' . $currencies->display_price($display_special_price, zen_get_tax_rate($product_check->fields['products_tax_class_id'])) . '</span></p>';
        }
        $show_sale_price = '';
      }
    } else {
