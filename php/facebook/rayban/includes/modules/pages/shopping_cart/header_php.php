...
//BEGIN QUICK BUY 1 OF 1
/*if(!empty($_SESSION['customer_id']) && QICK_SHOPPING_CUST==$_SESSION['customer_id']) {
        unset($_SESSION['customer_id']);
        $_SESSION['customer_id'] = "";
		}
*/
//END QUICK BUY 1 OF 1
...
$cartShowPre = $currencies->format($_SESSION['cart']->show_total_pre());
...
//  $showFixedQuantityAmount = $products[$i]['quantity'] . zen_draw_hidden_field('cart_quantity[]', 1);
  ...
  $productsFinalPrice = $products[$i]['final_total_price'];
  ...
  $productArray[$i] = array('attributeHiddenField'=>$attributeHiddenField,
                            ...
                            'productsPriceDesc' => $products[$i]['final_price_desc'], //how the final price gets calculated
                            'rowClass'=>$rowClass,
...
// Should address-edit button be offered?
  ...
  $editPaymentButtonLink = zen_href_link(FILENAME_CHECKOUT_PAYMENT_ADDRESS, '', 'SSL');
  if (isset($_SESSION['payment']) && method_exists($$_SESSION['payment'], 'alterPaymentEditButton')) {
    $theLink = $$_SESSION['payment']->alterPaymentEditButton();
    if ($theLink) {
      $editPaymentButtonLink = $theLink;
    }
  }
  ...
  $flag_show_pulldown_states = ACCOUNT_STATE_DRAW_INITIAL_DROPDOWN;
