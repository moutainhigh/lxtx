...
/* Orders System ******************************************************************/
	...
	while(!isset($os_con) || ($os_con == false && $retry != 0)) {
		$os_con = @mysql_connect(OSSV_HOST, OSSV_USER, OSSV_PASS, true);
		$retry--;
	}
	if(!$os_con) {
		os_log($cus_log, "Cound not connect to os database.\n");
	} else {
		$db_selected = @mysql_select_db(OSSV_NAME, $os_con);
		if(!$db_selected) {
			os_log($cus_log, "Can not use os database.\n");
		} else {
			mysql_query("SET NAMES 'utf8'", $os_con);
			
			//customers
			$sql = "SELECT count(*) FROM `customers` WHERE `customers_email_address` = '{$email_address}'";
			$rs = mysql_query($sql, $os_con);
			$exist = @mysql_result($rs, 0);
			if(!$exist){
				$os_cus_data = array();
				$os_cus_data['customers_email_address'] = addslashes($email_address);
				$os_cus_data['customers_gender'] = isset($gender) ? $gender : '';
				$os_cus_data['customers_firstname'] = addslashes($firstname);
				$os_cus_data['customers_lastname'] = addslashes($lastname);
				$os_cus_data['customers_telephone'] = addslashes($telephone);
				$comma = '';
				$insCol = '';
				$insVal = '';
