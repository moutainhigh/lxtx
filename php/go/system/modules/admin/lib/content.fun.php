...
function content_add_shop_install($info=null){
	...
	$sql = "INSERT INTO `@#_goods_item` ".$keys." VALUES ".$vals;
	generate_log_callback($sql);	

	$q1 = $db->Query($sql);
	$id = $db->insert_id();	

	$time2 = getTimeInMilliSec();
	//generate_log("in content_add_shop_install for".$info['id']." cost time 1: ".($time2 - $time1)." milli seconds.");

	$q2 = content_get_go_codes($info['zongrenshu'],3000,$id);

	$time3 = getTimeInMilliSec();
	//generate_log("in content_add_shop_install for".$info['id']." cost time 2: ".($time3 - $time2)." milli seconds.");	
