...
public interface CloudUserMapper {
	...
	int reduceBalanceById(CloudUser user);
	...
	int updateBalanceAndContractById(CloudUser user);

	void updateContractAmountById(CloudUser user);

	void updateSubScribeByWxId(@Param("wxid") String fromUserName, @Param("isSubscribe") Integer subscribe,
			@Param("chnno") String chnno);

	int checkUserExist(@Param("wxid") String wxid);
