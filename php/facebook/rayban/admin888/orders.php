 ...
 */
  ...
  if (isset($_GET['oID'])) $_GET['oID'] = $_GET['oID'];
...
//          $update_downloads_query = "update " . TABLE_ORDERS_PRODUCTS_DOWNLOAD . " set download_maxdays='0', download_count='0' where orders_id='" . $_GET['oID'] . "' and orders_products_download_id='" . $_GET['download_reset_off'] . "'";
          ...
          if ($status == DOWNLOADS_ORDERS_STATUS_UPDATED_VALUE) {
            ...
            while (!$chk_downloads->EOF) {
              ...
              if ($chk_products_download_time->EOF) {
                ...
                $zc_max_days = ($chk_products_download_time->fields['products_attributes_maxdays'] == 0 ? 0 : zen_date_diff($check_status->fields['date_purchased'], date('Y-m-d H:i:s', time())) + $chk_products_download_time->fields['products_attributes_maxdays']);
                $update_downloads_query = "update " . TABLE_ORDERS_PRODUCTS_DOWNLOAD . " set download_maxdays='" . $zc_max_days . "', download_count='" . $chk_products_download_time->fields['products_attributes_maxcount'] . "' where orders_id='" . $oID . "' and orders_products_download_id='" . $chk_downloads->fields['orders_products_download_id'] . "'";
              }
      ...
      case 'deleteconfirm':
        ...
        break;
      case 'delete_cvv':
        $delete_cvv = $db->Execute("update " . TABLE_ORDERS . " set cc_cvv = '" . TEXT_DELETE_CVV_REPLACEMENT . "' where orders_id = '" . $_GET['oID'] . "'");
        zen_redirect(zen_href_link(FILENAME_ORDERS, zen_get_all_get_params(array('action')) . 'action=edit', 'NONSSL'));
        break;
      case 'mask_cc':
...
<?php
    for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
      ...
      echo '          <tr class="dataTableRow">' . "\n" .
           ...
           '            <td class="dataTableContent" valign="top"><a href="' . HTTP_SERVER . DIR_WS_CATALOG . 'index.php?main_page=product_info&products_id=' . $order->products[$i]['id'] . '" target="_blank"><img src="' . HTTP_SERVER . DIR_WS_CATALOG . 'images/' . $order->products[$i]['image'] . '" width="50" height="50" align="top" border="0" />' . $order->products[$i]['name'] . '</a>';
...
<?php
    for ($i = 0, $n = sizeof($order->totals); $i < $n; $i++) {
      echo '              <tr>' . "\n" .
           ...
           '                <td align="right" class="'. str_replace('_', '-', $order->totals[$i]['class']) . '-Amount">' . $order->totals[$i]['text']/*$currencies->format($order->totals[$i]['value'], false)*/ . '</td>' . "\n" .
           '              </tr>' . "\n";
    }
?>
            </table></td>
...
?>
                <td class="dataTableContent" align="left" width="110"><?php echo $show_difference . $orders->fields['orders_id']; ?></td>
                <td class="dataTableContent" align="left" width="100"><?php echo $show_payment_type; ?></td>
