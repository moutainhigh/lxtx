...
class auto extends admin {	
	...
	public function show(){
		...
		$num=400;
		$total=$this->db->GetCount("SELECT COUNT(*) FROM `@#_shoplist` WHERE `q_uid` is null and `hot` < 2 order by `id` DESC"); 
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}	
		$page->config($total,$num,1,"0");
		$shoplist=$this->db->GetPage("SELECT * FROM `@#_shoplist` WHERE `q_uid` is null and `hot` < 2 and qishu <> -1 order by `id` DESC ",array("num"=>$num,"page"=>$p,"type"=>1,"cache"=>0));
