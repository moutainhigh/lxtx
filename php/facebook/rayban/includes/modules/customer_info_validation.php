...
$zco_notifier->notify('NOTIFY_MODULE_START_NO_ACCOUNT_VALIDATION');
...
$process = true;
    ...
    // collect posts to submit to the confirmation page
    include(DIR_WS_CLASSES . 'quick_checkout.php');
    $quick_checkout = new quick_checkout();
    $quick_checkout->collect_posts($_POST);
    
if (ACCOUNT_GENDER == 'true') {
  if (isset($_POST['gender'])) {
    $gender = zen_db_prepare_input($_POST['gender']);
	$gender_billing = $gender;
  } else {
    $gender_billing = $gender = false;
  }
}
...
$firstname_billing = $firstname;
$lastname_billing = $lastname;
$nick_billing = $nick;
...
$email_address_billing = $email_address;
$street_address_billing = $street_address;
...
if (FEC_CONFIRM_EMAIL == 'true') {
  ...
  if ($email_address != $email_address_confirm) {
    $error = true;
    $messageStack->add_session('no_account', ENTRY_EMAIL_ADDRESS_CONFIRM_ERROR);
  }
}
...
if (isset($_POST['password'])) {
  ...
  if (strlen($password) < ENTRY_PASSWORD_MIN_LENGTH) {
    $error = true;
    $messageStack->add_session('no_account', ENTRY_PASSWORD_ERROR);
  }
} elseif (FEC_NOACCOUNT_SWITCH == 'true') {
  // create password for no account
  $password=zen_db_prepare_input('dt7cA28');//zen_create_random_value(15, 'mixed');
  $cowoa = 0;
  $_SESSION['COWOA_CUSTOMER_PASSWORD'] = 'dt7cA28';// $password;
}
...
if (ACCOUNT_GENDER == 'true') {
  if ( ($gender != 'm') && ($gender != 'f') ) {
    $error = true;
    $messageStack->add_session('no_account', ENTRY_GENDER_ERROR);
  }
}

if (strlen($firstname) < ENTRY_FIRST_NAME_MIN_LENGTH) {
  $error = true;
  $messageStack->add_session('no_account', ENTRY_FIRST_NAME_ERROR);
}

if (strlen($lastname) < ENTRY_LAST_NAME_MIN_LENGTH) {
  $error = true;
  $messageStack->add_session('no_account', ENTRY_LAST_NAME_ERROR);
}
...
if (ACCOUNT_COMPANY == 'true') {
  if ((int)ENTRY_COMPANY_MIN_LENGTH > 0 && strlen($company) < ENTRY_COMPANY_MIN_LENGTH) {
    $error = true;
    $messageStack->add_session('no_account', ENTRY_COMPANY_ERROR);
  }
}
...
if (strlen($email_address) < ENTRY_EMAIL_ADDRESS_MIN_LENGTH) {
  $error = true;
  $messageStack->add_session('no_account', ENTRY_EMAIL_ADDRESS_ERROR);
} elseif (zen_validate_email($email_address) == false) {
  $error = true;
  $messageStack->add_session('no_account', ENTRY_EMAIL_ADDRESS_CHECK_ERROR);
} else {
  ...
  if ($check_email->fields['total'] > 0) {
    $error = true;
    $messageStack->add_session('no_account', ENTRY_EMAIL_ADDRESS_ERROR_EXISTS);
  }
}
...
if (strlen($street_address) < ENTRY_STREET_ADDRESS_MIN_LENGTH) {
  $error = true;
  $messageStack->add_session('no_account', ENTRY_STREET_ADDRESS_ERROR);
}
...
if (defined('PO_BOX_ERROR')) {
  if ( preg_match('/PO BOX/si', $street_address) ) {
    $error = true;
    $messageStack->add_session('no_account', PO_BOX_ERROR);
...
if (strlen($city) < ENTRY_CITY_MIN_LENGTH) {
  $error = true;
  $messageStack->add_session('no_account', ENTRY_CITY_ERROR);
}
...
if (ACCOUNT_STATE == 'true') {
  ...
  if ($entry_state_has_zones == true) {
    ...
    if ($found_exact_iso_match) {
      ...
      $messageStack->add_session('no_account', ENTRY_STATE_ERROR_SELECT);
    }
  } else {
    if (strlen($state) < ENTRY_STATE_MIN_LENGTH) {
      $error = true;
      $error_state_input = true;
      $messageStack->add_session('no_account', ENTRY_STATE_ERROR);
    }
  }
}
...
if (strlen($postcode) < ENTRY_POSTCODE_MIN_LENGTH) {
  $error = true;
  $messageStack->add_session('no_account', ENTRY_POST_CODE_ERROR);
}

if ( (is_numeric($country) == false) || ($country < 1) ) {
  $error = true;
  $messageStack->add_session('no_account', ENTRY_COUNTRY_ERROR);
}

if (ACCOUNT_TELEPHONE == 'true') {
  if (strlen($telephone) < ENTRY_TELEPHONE_MIN_LENGTH) {
    $error = true;
    $messageStack->add_session('no_account', ENTRY_TELEPHONE_NUMBER_ERROR);
  }
}
...
$zco_notifier->notify('NOTIFY_MODULE_END_NO_ACCOUNT_VALIDATION');
?>