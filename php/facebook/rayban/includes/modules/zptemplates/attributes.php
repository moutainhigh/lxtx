...
if ($check_grid) {
  ...
  $attrib_grid = '<table id="attrib-grid-table" cellpadding="0" cellspacing="0">'."\n" . 
  				 '<tr>'."\n".
				 '<td colspan="'.sizeof($grid_records['H']['options']).'" class="attrib-grid-opt-nameH">'.$grid_records['H']['name'].(zen_not_null($grid_records['H']['comment']) ? '<br /><span id="attrib-grid-opt-commentH">'.$grid_records['H']['comment'].'</span>' : '').'</td>'."\n";
	
	for($grv=0; $grv < $grv_size; $grv++) {
	if ($grv == 0) {
	  $attrib_grid .= '  <td rowspan="'.$grv_size.'" class="attrib-grid-opt-nameV">'.$grid_records['V']['name'].(zen_not_null($grid_records['H']['comment']) ? '<br /><span id="attrib-grid-opt-commentV">'.$grid_records['V']['comment'].'</span>' : '').'</td>'.'</tr>'."\n";
	}
	if (($grv_size > 1) || zen_not_null($grid_records['V']['options'][$grv]['name'])) {
	  $attrib_grid .= '  <td class="attrib-grid-vHeader" id="attrib-grid-vHeader-'.$grid_records['V']['options'][$grv]['id'].'">'.$grid_records['V']['options'][$grv]['name']. ($grid_records['V']['options'][$grv]['price'] != 0 ? '<br /><span class="attrib-grid-header-price">'.$grid_records['V']['options'][$grv]['price_prefix'].$currencies->format($grid_records['V']['options'][$grv]['price']).'</span>' : '').'</td>'.'</tr>'."\n";
	}
	}
  ...
  for($grv=0; $grv < $grv_size; $grv++) {
	  ...
	  $attrib_grid .= '<tr class="attrib-grid-hHeader" id="attrib-grid-hHeader-'.$grid_records['H']['options'][$grh]['id'].'"><td colspan="'.sizeof($grid_records['H']['options']).'" class="attrib-grid-opt-nameH">'.$grid_records['H']['options'][$grh]['name']. ($grid_records['H']['options'][$grh]['price'] != 0 ? '<br /><span class="attrib-grid-header-price">'.$grid_records['H']['options'][$grh]['price_prefix'].$currencies->format($grid_records['H']['options'][$grh]['price']).'</span>' : ''.'</td>');
	...
	$show_attribute_out_of_stock = ($attribute_stock_controlled && SHOW_PRODUCTS_SOLD_OUT_IMAGE == '1');
	  ...
	  $attrib_grid .= '<td class="attrib-grid-cell" id="attrib-grid-cell-'.$grid_records['H']['options'][$grh]['id'].'-'.$grid_records['V']['options'][$grv]['id'].'">';
		/*2012-03-22***************************************************************/
		$attrib_grid .= '<div class="cart_qty reduce_qty"><a href="javascript:void(0);">-</a></div>';
		/****************************************************************/
	  switch(true) {
		...
		break;
	    default:
		  $attrib_grid .=	zen_draw_input_field('products_id['.$products_attribs_id.']', '', 'size="3" value="0" class="inpQty"').
		...
		/*2012-03-22***************************************************************/
		$attrib_grid .= '<div class="cart_qty add_qty"><a href="javascript:void(0);">+</a></div>';
		/****************************************************************/
	  $attrib_grid .= '</td>'.'</tr>'."\n";
...
//设置要显示的产品属性: popt.products_options_id > 7
              ...
              from        " . TABLE_PRODUCTS_OPTIONS . " popt, " . TABLE_PRODUCTS_ATTRIBUTES . " patrib
              where           patrib.products_id='" . (int)$_GET['products_id'] . "'
              and			  popt.products_options_id > 0
              and             patrib.options_id = popt.products_options_id
              and             popt.language_id = '" . (int)$_SESSION['languages_id'] . "' " .
              $options_order_by;
                    ...
                    if ($_POST['id']) {
                      ...
                      if ( $products_options_names->fields['products_options_rows'] > 1 ) {
                        ...
                        //                $tmp_html .= '  <input type="reset">';
					  	...
					  	if ($products_options_names->fields['products_options_name'] == 'Name') { $options_extra = ' reg="^\w{1,14}$"'; } 
						elseif ($products_options_names->fields['products_options_name'] == 'Number') { $options_extra = ' tip="Please enter only digits" reg="^\d{1,2}$"'; }
						else { $options_extra = ''; }

                        $tmp_html = '<input type="text" name="id[' . TEXT_PREFIX . $products_options_names->fields['products_options_id'] . ']" size="' . $products_options_names->fields['products_options_size'] .'" maxlength="' . $products_options_names->fields['products_options_length'] . '" value="' . htmlspecialchars($tmp_value) .'" id="' . 'attrib-' . $products_options_names->fields['products_options_id'] . '-' . $products_options_value_id . '"' . $options_extra . ' />  ';
                      }
                      $tmp_html .= $products_options_details;
                      $tmp_word_cnt_string = '';
...
/****************************************************/
                  ...
                  $options_menu[] = zen_draw_pull_down_menu('id[' . $products_options_names->fields['products_options_id'] . ']', $products_options_array, $selected_attribute, 'id="' . 'attrib-' . $products_options_names->fields['products_options_id'] . '" reg="[^0]"') . "\n";
                  $options_comment[] = $products_options_names->fields['products_options_comment'];
                  $options_comment_position[] = ($products_options_names->fields['products_options_comment_position'] == '1' ? '1' : '0');
                  break;
                }
