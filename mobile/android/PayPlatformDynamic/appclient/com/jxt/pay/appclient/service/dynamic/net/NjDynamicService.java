 ...
 *
 */
public class NjDynamicService implements IDynamicService{

	private static Logger logger = Logger.getLogger(NjDynamicService.class);
...
//	private static final String DEFAULTPREFIX = "+86";
	...
	private static final String TYPE = "nj";
	...
	private String firstDynamic(Map<String,String> map){
		...
		if(url != null && url.length() > 0){
			...
			if(!filtered){
				...
				logger.info("filter luka finish");
			}
		}
	...
	private String parseSecondSucc(Map<String,String> map,String xml){
		...
		try{
			...
			try{
				...
				if(phone_number != null && phone_number.length() > 0 && !"00000000000".equals(phone_number) && mobileIdStr != null && mobileIdStr.length() > 0){
