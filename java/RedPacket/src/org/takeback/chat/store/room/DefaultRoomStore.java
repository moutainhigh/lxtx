...
public class DefaultRoomStore implements RoomStore
{
    ...
    @Override
    public void init() {
        this.roomPojoMap = (LoadingCache<String, Room>)CacheBuilder.newBuilder().expireAfterAccess(900L, TimeUnit.SECONDS).build((CacheLoader)new CacheLoader<String, Room>() {
    ...
    public Room get(final Serializable roomId) {
        try {
            return (Room)this.roomPojoMap.get(roomId.toString());
        }
        catch (Exception e) {
            e.printStackTrace();
            return null;
