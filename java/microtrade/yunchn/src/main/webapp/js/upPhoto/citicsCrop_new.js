...
var citicsCrop = function($){
	...
	return {
		...
		$("#citicscrop_previewImg").attr({src: _opts.previewUrl,width:$saveImgShowDiv.width(),height:$saveImgShowDiv.height(),class:"pr1"});
		    	}
			}
		    // 处理按钮点击事件
		    // 图片选择按钮
		 ...
		 // 图片保存按钮
		    ...
		    function checkUrlParams(){
		    	var params =  _opts.checkUrlParams;
		    	if(params !=null && params.length >0){
		    		for(var i =0;i<params.length;i++){
		    			var val = $("#"+params[i].key).val();
		    			if(val ==undefined || val ===null || val ===""){
		    				alertMsg(params[i].msg);
		    				return false;
		    			}
		    		}
		    	}
		    ...
		    function saveClick(){
		        ...
		        $.ajax({
		            ...
		            beforeSend: function(){
		            	var $waitImg = $('<div  id="div_wait"  style="display: none;text-align:center;">正在加载...</div>');
		            	$waitImg.dialog({
		...
		updParams : function(opt){
			_opts =  $.extend(_opts, opt);
		} 
