...
//      $this->_log($errorMessage . print_r($myorder, true) . print_r($mylphp->xmlString, true), $myorder["oid"] . ($failure ? '_FAILED' : ''));
  ...
  function _doRefund($oID, $amount = 0) {
    ...
    $sql = "select lp_trans_num, transaction_time from " . TABLE_LINKPOINT_API . " where order_id = " . $oID . " and transaction_result = 'APPROVED' order by transaction_time DESC";
  ...
  function _doCapt($oID, $amt = 0, $currency = 'USD') {
    ...
    $sql = "select lp_trans_num, chargetotal from " . TABLE_LINKPOINT_API . " where order_id = " . $oID . " and transaction_result = 'APPROVED' order by date_added";
  ...
  function _doVoid($oID, $note = '') {
    ...
    $sql = "select lp_trans_num, transaction_time from " . TABLE_LINKPOINT_API . " where order_id = " . $oID . " and transaction_result = 'APPROVED' order by date_added";
    $query = $db->Execute($sql);
    if ($query->RecordCount() < 1) {
      $messageStack->add_session(MODULE_PAYMENT_LINKPOINT_API_TEXT_NO_MATCHING_ORDER_FOUND, 'error');
      $proceedToVoid = false;
