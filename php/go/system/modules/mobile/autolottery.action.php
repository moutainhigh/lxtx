   ...
   //限时揭晓 明日
	...
	public function autolottery_ret_install(){
		...
		if($u_go_info){			
				...
				$q_1 = $this->db->Query("UPDATE `@#_goods_item` SET								
			...
			writeContentById($id, "q_content_", $content);
	...
	private function autolottery_install($shop=null){
		...
		if($shop['qishu'] < $shop['maxqishu']){		
			...
			$q_1 = $this->db->Query("INSERT INTO `@#_goods_item` (`sid`, `zongrenshu`, `canyurenshu`,`shenyurenshu`, `qishu`,`maxqishu`, `codes_table`,`xsjx_time`,`renqi`,`pos`, `time`)


					VALUES


					('$goods[sid]','$goods[zongrenshu]','$goods[def_renshu]','$shenyurenshu', '$qishu','$goods[maxqishu]', '$query_table','0','$goods[renqi]','$goods[pos]','$time')
