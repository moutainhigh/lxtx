...
Processing...
  ...
  switch ($txn_type) {
    ...
    case (($txn_type == 'express_checkout' || $isECtransaction) && !strstr($txn_type, 'cleared') && $parentLookup != 'parent'):
      ...
      if (!(substr($txn_type,0,8) == 'pending-' && $ordersID <= 0) && !($new_record_needed && $txn_type == 'echeck-cleared') && $txn_type != 'unique' && $txn_type != 'echeck-denied' && $txn_type != 'voided') {
    ...
    case (substr($txn_type,0,8) == 'pending-' && $ordersID <= 0):
  ...
  switch (TRUE) {
    ...
    case ($txn_type == 'pending-multicurrency' && $ordersID > 0):
      ...
      break;
    case ($txn_type == 'pending-address' && $ordersID > 0):
      ...
      break;
    case ($txn_type == 'pending-paymentreview' && $ordersID > 0):
