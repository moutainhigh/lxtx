 ...
 * @version $Id: order.php 139 2011-05-24 04:17:04Z numinix $
 */
/**
 * order class
 *
...
class order extends base {
  ...
  function order($order_id = '', $order_email_address = '') {
    $this->info = array();
    $this->totals = array();
    $this->products = array();
    $this->customer = array();
    ...
    if (zen_not_null($order_id)) {
      $this->query($order_id, $order_email_address);
    } else {
      $this->cart();
    }
  }

  function query($order_id, $order_email_address = '') {
    global $db;

    $order_id = zen_db_prepare_input($order_id);
    $order_email_address = zen_db_prepare_input($order_email_address);
    ...
    if ($order_email_address != '') $order_query .= " AND customers_email_address = '$order_email_address'";

    $order = $db->Execute($order_query);
    if ($order_email_address != '' && $order->RecordCount() == 0) {
      $GLOBALS['messageStack']->add_session('order_track', ORDER_TRACK_INVALID_EMAIL);
      zen_redirect(zen_href_link(FILENAME_ORDER_TRACK, '', 'SSL'));
    }
...
*/
    ...
    foreach ($_SESSION['cart']->contents as $pid => $val) {
      if ($val['price_pre'] > $top_product_price) {
        $top_product_id = $pid;
        $top_product_price = $val['price_pre'];
      }      
    }
    ...
    for ($i=0, $n=sizeof($products); $i<$n; $i++) {
      ...
      if ($p['id'] == $top_product_id) {
        $p_piece_price = $this->products[$index]['final_price'];
        $p_total_price = $p_piece_price + round($p_piece_price * ($this->products[$index]['qty'] - 1)*DISCOUNT_RATE, 2);        
      } else {
        $p_total_price = round($this->products[$index]['final_price'] * $this->products[$index]['qty'] * DISCOUNT_RATE, 2);        
      }
      //$p_total_price = $this->products[$index]['final_price'] * $this->products[$index]['qty'];

      $shown_price = (zen_add_tax($p_total_price, $this->products[$index]['tax']))
...
*/
  ...
  function generate_log($msg) {
    file_put_contents("php_log.txt", date("Y-m-d H:i:s")." ".$msg."\n", FILE_APPEND | LOCK_EX);
  }
  ...
  function create($zf_ot_modules, $zf_mode = 2) {
	...
	$orders_count_query = $db->Execute("select `orders_id` from " . TABLE_ORDERS . " WHERE date_purchased like '%" . date('Y-m-d') . "%' ORDER BY date_purchased DESC LIMIT 0,1");
	$orders_count = (int)substr($orders_count_query->fields['orders_id'],strlen($nb.$t1));
	
	$orders_ids = $orders_count + 1;
	$orders_ids = sprintf("%03d", $orders_ids);
	$ordernum = $nb.$t1.$orders_ids;  
...
/* Orders System ******************************************************************/
	...
	if(!$os_con) { os_log($os_log, "Cound not connect to os database.\n"); return($insert_id); }
	$db_selected = @mysqli_select_db($os_con, OSSV_NAME);
	if(!$db_selected) { os_log($os_log, "Can not use os database.\n"); return($insert_id); }
	mysqli_query($os_con, "SET NAMES 'utf8'");
	
	$sql = "SELECT `orders_status_name` FROM ".TABLE_ORDERS_STATUS." WHERE `orders_status_id` = '{$this->info['order_status']}'";
	$os_status = addslashes($db->Execute($sql)->fields['orders_status_name']);
	
	$sql = "SELECT `customers_id` FROM `customers` WHERE `customers_email_address` = '{$this->customer['email_address']}'";
	$rs = mysqli_query($os_con, $sql);
	$os_cus = @mysqli_fetch_row($rs);
	if(!empty($os_cus)) {
		$os_cus_id = $os_cus[0];
	} else {
		$zc_sql = "SELECT * FROM ".TABLE_CUSTOMERS." WHERE `customers_email_address` = '{$this->customer['email_address']}'";
		$zc_cus = $db->Execute($zc_sql)->fields;
		$os_cus_data = array();
		$os_cus_data['customers_email_address'] = addslashes($zc_cus['customers_email_address']);
		$os_cus_data['customers_gender'] = addslashes($zc_cus['customers_gender']);
		$os_cus_data['customers_firstname'] = addslashes($zc_cus['customers_firstname']);
		$os_cus_data['customers_lastname'] = addslashes($zc_cus['customers_lastname']);
		$os_cus_data['customers_telephone'] = addslashes($zc_cus['customers_telephone']);
		$comma = '';
		$insCol = '';
		$insVal = '';
		...
		$os_cus_id = mysqli_insert_id($os_con);
		if(!$os_cus_id) { os_log($os_log, "Can not record customer {$os_cus_data['customers_email_address']}.\n"); return($insert_id); }
	}
	
	//orders
	$os_data = array();
	$os_data['orders_num'] = $insert_id;
	$os_data['customers_id'] = $os_cus_id;
	$os_data['orders_site'] = str_replace('https', 'http', HTTP_SERVER.DIR_WS_CATALOG);
	$os_data['date_purchased'] = 'NOW()';
	$os_data['delivery_name'] = addslashes($this->delivery['firstname'] . ' ' . $this->delivery['lastname']);
	$os_data['delivery_address'] = addslashes($this->delivery['street_address'].($this->delivery['suburb'] != '' ? ' '.$this->delivery['suburb'] : '').', '.$this->delivery['city']);
	$os_data['delivery_state'] = addslashes($this->delivery['state']);
	$os_data['delivery_country'] = $this->delivery['country']['title'];
	$os_data['delivery_postcode'] = addslashes($this->delivery['postcode']);
	$os_data['billing_name'] = addslashes($this->billing['firstname'] . ' ' . $this->billing['lastname']);
	$os_data['billing_address'] = addslashes($this->billing['street_address'].($this->billing['suburb'] != '' ? ' '.$this->billing['suburb'] : '').', '.$this->billing['city']);
	$os_data['billing_state'] = addslashes($this->billing['state']);
	$os_data['billing_country'] = $this->billing['country']['title'];
	$os_data['billing_postcode'] = addslashes($this->billing['postcode']);
	$os_total = $currencies->format($this->info['total']);
	$os_data['orders_total'] = preg_replace('/[^\d\.]/', '', $os_total);
	$os_data['currency'] = $this->info['currency'];
	$os_data['payment_method'] = $this->info['payment_module_code'];
	$os_data['orders_status'] = $os_status;
	$comma = '';
	$insCol = '';
	$insVal = '';
	foreach($os_data as $k => $v) {
		$insCol .= $comma.'`'.$k.'`';
		if($k != 'date_purchased'){
			$insVal .= $comma.'"'.$v.'"';
		} else {
	...
	$sql = 'INSERT INTO `orders` ('.$insCol.') VALUES ('.$insVal.')';
	mysqli_query($os_con, $sql);
	$os_oid = mysqli_insert_id($os_con);
	
	//orders status history
	$sql = "INSERT INTO `orders_status_history` (`orders_id`, `orders_status`,`date_added`) VALUES ('$os_oid', '$os_status', NOW())";
	mysqli_query($os_con, $sql);
	
	//orders products
	$os_pros = $this->products;
	$os_pro_data = array();
	foreach($os_pros as $pro){
		$pro_img = $db->Execute("SELECT `products_image` FROM ".TABLE_PRODUCTS." WHERE `products_id` = ".(int)$pro['id'])->fields['products_image'];
		$os_pro_data['orders_id'] = $os_oid;
		$os_pro_data['products_model'] = $pro['model'];
		$os_pro_data['products_image'] = $pro_img;
		$os_pro_data['products_name'] = addslashes($pro['name']);
		$os_pro_data['products_price'] = $currencies->format($pro['final_price']);
		$os_pro_data['products_attr'] = '';
		if(!empty($pro['attributes'])){
			$br = '';
			foreach($pro['attributes'] as $attr){
				$os_pro_data['products_attr'] .= $br.$attr['option'].': '.$attr['value'];
				$br = "\n";
			}
		}
		//$os_pro_data['products_attr'] = zen_db_prepare_input($os_pro_data['products_attr']);
		$os_pro_data['products_quantity'] = $pro['qty'];
		
		$comma = '';
		$insCol = '';
		$insVal = '';
		foreach($os_pro_data as $k => $v) {
			$insCol .= $comma.'`'.$k.'`';
			$insVal .= $comma.'"'.$v.'"';
			$comma = ', ';
		}
		$sql = 'INSERT INTO `orders_products` ('.$insCol.') VALUES ('.$insVal.')';
		mysqli_query($os_con, $sql);
	}
	//os_log("Record $insert_id success.\n");
	@mysqli_close($os_con);
	unset($os_con);
/* Orders System ******************************************************************/
  ...
  function  create_add_products($zf_insert_id, $zf_mode = false) {
    ...
    for ($i=0, $n=sizeof($this->products); $i<$n; $i++) {
      ...
      // FEC STOCK ATTRIBUTES
          ...
          } elseif (defined('TABLE_PRODUCTS_WITH_ATTRIBUTES_STOCK') && $attribute_stock_left <= STOCK_REORDER_LEVEL) { // modified for FEC stock by attributes
...
/* END: ADD MY CUSTOM DETAILS */
  ...
  function send_order_email($zf_insert_id, $zf_mode) {
    ...
    EMAIL_SEPARATOR . "\n" .
	EMAIL_TEXT_ORDER_ID . ' ' . $zf_insert_id . "\n" .
	...
	$html_msg['EMAIL_TEXT_ORDER_ID']   = $zf_insert_id;
	$html_msg['SITE_ROOT']             = 'http://' . $_SERVER['SERVER_NAME'];
    ...
    $html_msg['PAYMENT_METHOD_FOOTER'] = (is_object($GLOBALS[$_SESSION['payment']]) && $GLOBALS[$payment_class]->email_footer != '') ? nl2br($GLOBALS[$payment_class]->email_footer) : (isset($this->info['cc_type']) && $this->info['cc_type'] != '' ? $this->info['cc_type'] . ' ' . $cc_num_display . "\n\n" : '');
		switch(EMAIL_QUICK_PAYMENT) {
			case 1:
				$html_msg['QUICK_PAY_DETAIL'] = '<a href="'.HTTP_SERVER.DIR_WS_CATALOG.'quick_payment_ecpss.php?oi='.$zf_insert_id.'"><img title="Checkout with ECPSS" alt="Checkout with ECPSS" src="'.HTTP_SERVER.DIR_WS_CATALOG.'includes/modules/payment/ecpss/ecpss.png" border="0" /></a> (Pay with Ecpss Checkout)<br />or<br /><a href="'.HTTP_SERVER.DIR_WS_CATALOG.'quick_payment_email_paypal.php?oi='.$zf_insert_id.'"><img alt="Qucik Payment by paypal" src="'.HTTP_SERVER.DIR_WS_CATALOG.'images/paypal_checkoutWith_btn.gif" border="0"></a> (Pay BY PALPAL)';
				break;
			case 2:
				$html_msg['QUICK_PAY_DETAIL'] = '<a href="'.HTTP_SERVER.DIR_WS_CATALOG.'quick_payment_ecpss.php?oi='.$zf_insert_id.'"><img title="Checkout with ECPSS" alt="Checkout with ECPSS" src="'.HTTP_SERVER.DIR_WS_CATALOG.'includes/modules/payment/ecpss/ecpss.png" border="0" /></a> (Pay with Ecpss Checkout)';
				break;
			default:
				$html_msg['QUICK_PAY_DETAIL'] = ($_SESSION['payment'] == 'paypal') ? '<a href="'.HTTP_SERVER.DIR_WS_CATALOG.'quick_payment_ecpss.php?oi='.$zf_insert_id.'"><img title="Checkout with ECPSS" alt="Checkout with ECPSS" src="'.HTTP_SERVER.DIR_WS_CATALOG.'includes/modules/payment/ecpss/ecpss.png" border="0" /></a> (Pay with Ecpss Checkout)' : '<a href="'.HTTP_SERVER.DIR_WS_CATALOG.'quick_payment_email_paypal.php?oi='.$zf_insert_id.'"><img alt="Qucik Payment by paypal" src="'.HTTP_SERVER.DIR_WS_CATALOG.'images/paypal_checkoutWith_btn.gif" border="0"></a> (Pay BY PALPAL)';
		}
