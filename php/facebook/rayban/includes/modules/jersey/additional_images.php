...
if ($products_image != '') {
  ...
  //$images_array[] = str_replace($products_image_directory, '', DIR_WS_IMAGES.$products_image);
...
if ($num_images) {
  ...
  for ($i=0, $n=$num_images; $i<$n; $i++) {
    ...
    $thumb_slashes = zen_image($base_image, addslashes($products_name), 100, '');
    // remove additional single quotes from image attributes (important!)
    $thumb_slashes = preg_replace("/([^\\\\])'/", '$1\\\'', $thumb_slashes);
    $thumb_regular = zen_image_OLD($base_image, $products_name, 100, '');
    ...
    $script_link = '<a href="'. zen_lightbox($products_image_large, addslashes($products_name), LARGE_IMAGE_MAX_WIDTH, LARGE_IMAGE_MAX_HEIGHT) . '" class="'.$css_class.'">' . $thumb_regular . '</a>';
    }
    else 
    ...
    $list_box_contents[$row][$col] = array('params' => 'class="scrolleritem"',
    'text' => "\n	<div class=\"adtnImgBox\"><div class=\"adtnImg\">".$link."</div></div>");
    $col ++;
