...
class fund extends admin {
	...
	public function specify(){
		if(isset($_POST['dosubmit'])){
			...
			$res1 = $this->db->Query("UPDATE `@#_goods_item` SET `zhiding` = '$userid' WHERE `id`='$shopid'");
			if($res>0 && $res1>0){
	...
	public function zddel(){
		...
		$res1 = $this->db->Query("UPDATE `@#_goods_item` SET `zhiding` = 0 WHERE `sid`='{$shopinfo['sid']}' AND `q_uid` is Null");
			if($res>0 && $res1>0){			
				_message("删除成功");
			}else{
				_message("删除失败");
