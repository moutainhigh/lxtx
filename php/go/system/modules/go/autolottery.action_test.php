...
class autolottery extends SystemAction {
	...
	public function autolottery_ret_install(){	
		...
		$cp_info = $this->db->GetOne("select * from `@#_caipiao` where `id` = '$id' for update");
	//	print_r($cp_info);
		...
		$tocode->run_tocode($shop_info['xsjx_time'],$cp_info['kj_num'],$shop_info['canyurenshu']);
		$code =$tocode->go_code;				
		...
		if($u_go_info){			
				...
				$q_1 = $this->db->Query("UPDATE `@#_shoplist` SET								
								...
								`q_showtime` = 'Y'
								 where `id` = '$id' and `webid`='$_COOKIE[web_id]'");
								 
			if($u_go_info){
					$q_2 = $this->db->Query("UPDATE `@#_member_go_record` SET `huode` = '$code' where `id` = '$u_go_info[id]' and `webid`='$_COOKIE[web_id]'");
			}else{
					$q_2 = true;
			}
	...
	private function autolottery_install($shop=null){
		if($shop['qishu'] < $shop['maxqishu']){		
			...
			$q_1 = $this->db->Query("INSERT INTO `@#_shoplist` (`sid`,`cateid`, `isgh`,`brandid`, `title`, `title_style`, `title2`, `keywords`, `description`, `money`, `yunjiage`, `zongrenshu`, `canyurenshu`,`shenyurenshu`,`def_renshu`, `qishu`,`maxqishu`,`thumb`, `picarr`, `content`,`codes_table`,`xsjx_time`,`renqi`,`pos`, `time`,`webid`)
					VALUES
					('$goods[sid]','$goods[cateid]','$goods[isgh]','$goods[brandid]','$goods[title]','$goods[title_style]','$goods[title2]','$goods[keywords]','$goods[description]','$goods[money]','$goods[yunjiage]','$goods[zongrenshu]','$goods[def_renshu]','$shenyurenshu','$goods[def_renshu]','$qishu','$goods[maxqishu]','$goods[thumb]','$goods[picarr]','$goods[content]','$query_table','0','$goods[renqi]','$goods[pos]','$time','$_COOKIE[web_id]')
					");				
