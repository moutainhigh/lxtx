...
<script type="text/javascript">	
	...
	jQuery(function($){
		...
		jQuery(grid_selector).jqGrid({
			...
			mtype:'GET',
			height: '420',
			colNames:[ '','渠道编号','时间','','佣金','注册用户数','订单数','盈','亏','交易金额','总盈亏','操作'],
			colModel:[
				...
				{         name:'date1', index:'date1', align:'center',hidden:true,width:10,editable:false,sortable:false},//排序根据此index
				...
				{         name:'contractMoney', index:'contractMoney', width:10,editable:false,sortable:false},
				{         name:'fProfit', index:'fProfit', width:10,editable:false,sortable:false},
				{         name:'id', index:'id', align:'center', width:20,editable:false,title:false,formatter:setOper,sortable:false},//排序根据此index
			], 
			viewrecords : true,
			rowNum:10,
			rowList:[10,20,30],
...
// 				$("#grid-table").jqGrid("addRowData", "id", data.obj, "first");
	...
	function setOper(el, cellval, opts){
		var oper = "";
		if(opts.chnno!=null){
			oper += "<span id=\"tt\" title=\"历史数据\" class=\"oprbutton blue\" onclick=\"hisObj('"+opts.chnno+"')\" ><p>历史数据</p></span>";
			if("${userProfitFlag=='on'}"=='true'){
	// 			oper += "<span id=\"tt\" title=\"分时数据\" class=\"oprbutton green\" onclick=\"perObj('"+opts.chnno+"','"+opts.date1+"')\" ><p>分时数据</p></span>";
				oper += "<span id=\"tt\" title=\"订单数据\" class=\"oprbutton green\" onclick=\"orderObj('"+opts.chnno+"')\" ><p> 订单数据</p></span>";
				oper += "<span id=\"tt\" title=\"入金数据\" class=\"oprbutton blue\" onclick=\"fillObj('"+opts.chnno+"')\" ><p> 入金数据</p></span>";
				
			}
		}
		return oper;
	}
	function setContract(el, cellval, opts){
		return Number((opts.profit-opts.loss).toFixed(2));
	}
	
	function hisObj(id){
		location.href="${basePath}/chn/enterChnHistory?chnno="+id;
	}
	
	function perObj(id,date){
		location.href="${basePath}/chn/enterChnPerTime?chnno="+id+"&date="+date;
	}
	
	function orderObj(id){
		location.href="${basePath}/chn/enterChnOrder?chnno="+id;
	}
	function fillObj(id){
		location.href="${basePath}/chn/enterChnFill?chnno="+id;
	}
	function reload() {//查询用
		if($("#startDate").val()==""){
			alertMsg("查询时间不能为空！");
