...
class category_tree extends base {
 ...
 function zen_category_sub($categoriesid = 0) {
     global $db, $cPath, $cPath_array;
      ...
      $categories_query = "select c.categories_id, cd.categories_name, c.parent_id, c.categories_image
                             ...
                             where c.parent_id = ".$categoriesid."
		...
		$curtree = array();		
          ...
          //$new_path .= $value;
          while (!$rows->EOF) {
            $curtree[$rows->fields['categories_id']] = array('name' => $rows->fields['categories_name'],
            'parent' => $rows->fields['parent_id'],
            'level' => $key+1,
            'path' => $categoriesid > 0 ?$categoriesid . '_' . $rows->fields['categories_id']:$rows->fields['categories_id'],
            'image' => $categories->fields['categories_image'],
            'next_id' => false);

            if (isset($parent_id)) {
               $curtree[$parent_id]['next_id'] = $rows->fields['categories_id'];
            }
           ...
           $curtree[$last_id]['next_id'] = $curtree[$value]['next_id'];
           $curtree[$value]['next_id'] = $first_id;
          $new_path .= '_';
        } 
