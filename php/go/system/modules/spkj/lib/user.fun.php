...
function get_ip($id,$ipmac=null){
	...
	$record=$db->GetOne("select * from `@#_member_go_record_shipin` where `id`='$id'");
...
function Getheader($type='index'){
	...
	//$navigation=$mysql_model->GetList("select * from `@#_navigation` where `status`='Y'  and `type` = '$type' and `webid`='".WEB_ID."' order by `cid` asc");
	$url="";
	if($type=='foot'){
		foreach($navigation as $v){		
		//	$url.='<a  href="'.WEB_PATH.$v['url'].'">'.$v['name'].'</a>';
			$url.='<li><a href="'.WEB_PATH.$v['url'].'">'.$v['name'].'</a></li><li>|</li>';
		}
		return $url;
	}
	foreach($navigation as $v){		
