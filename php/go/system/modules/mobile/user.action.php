...
class user extends memberbase {
	...
	public function wx_init(){
		$web_path = G_WEB_PATH.'/index.php/mobile/user';
	...
	public function callback(){
		...
		if($status!='success' || empty($openid)){
			_messagemobile('授权失败',WEB_PATH.'/mobile/home');
			exit();
		}
		...
		$go_user_himg  = $user_info['headimgurl'];
		if(empty($weixin_openid)){
			_messagemobile('信息获取失败，请重新操作',WEB_PATH.'/mobile/home');
			exit();
		}
		$info = $this->db->GetOne("SELECT * FROM `@#_member_band` WHERE `b_code` = '$weixin_openid'");
		if(!empty($info)){
	...
	public function mobilesuccess(){
		...
		$member2=$this->db->GetOne("select mobilecode,uid,mobile from `@#_member` where `mobile`='$mobile'");
			...
			$uid = $member['uid'];
			generate_log_weixin("uid is:".$uid);
			$this->db->Query("UPDATE `@#_member` SET mobilecode='$checkcodes' where `uid`='$uid'");				
			$checkcodes = explode("|",$checkcodes);

			generate_log_weixin("valid code is:".$checkcodes);

			//$ok = send_mobile_reg_code($mobile,$member['uid']);			
			$ok = send_valid_code($checkcodes[0], $mobile);
			if($ok != 0){
	...
	public function error() {
		_messagemobile('请在APP内进行绑定',WEB_PATH.'/mobile/user/login');
		exit();
	}
