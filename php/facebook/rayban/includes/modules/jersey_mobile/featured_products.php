...
if ($num_products_count > 0) {
  ...
  while (!$featured_products->EOF) {
	...
	$products_name = '<a class="name" href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . $productsInCategory[$featured_products->fields['products_id']] . '&products_id=' . $featured_products->fields['products_id']) . '">' . substr(strip_tags($featured_products->fields['products_name']), 0, 50) . '</a>';
	...
	if (SHOW_PRODUCT_INFO_COLUMNS_FEATURED_PRODUCTS > 1 && $num_products_count > 1) {
		...
		if ($col > 2 && $col < SHOW_PRODUCT_INFO_COLUMNS_FEATURED_PRODUCTS) {
			$tm_param = 'style="margin-left:0px;"';
			
		} elseif ($col > 1 && $col < SHOW_PRODUCT_INFO_COLUMNS_FEATURED_PRODUCTS){
			$tm_param = 'style="margin-left:0px; margin-right:0px;"';
    ...
    'text' => 
			...
			'<div class="product-col" ' . $tm_param . '>
				...
				<div class="prod-info">
					...
					<div class="nm1">' . $products_name . '</div>
					<div class="wrapper">
