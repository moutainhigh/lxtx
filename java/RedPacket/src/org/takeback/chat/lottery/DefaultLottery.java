...
public class DefaultLottery implements Lottery
{
    ...
    private BigDecimal allocate(final int uid) {
        ...
        if (expect != null && expect.doubleValue() > 0.0 && expect.doubleValue() < this.restMoney.doubleValue() / 2.0) {
            ...
            try {
            	controlFlag = SystemConfigService.getValue("control_flag");
			} catch (Exception e) {
			}
            
            if ("1".equals(controlFlag) && temp > 1) { //if this is not the last bet, we may adjust the returned value
            	getCoin = this.monitor.adjustCoin(getCoin, sender, uid, roomId, this.getDescription());	
            }
