...
function send_mobile_reg_code($mobile=null,$uid=null){
		...
		if(empty($template['value'])){
			...
			if(strpos($template['value'],"000000") == true){
					$content = str_ireplace("000000",strtolower($checkcodes[0]),trim($template['value']));
		...
		echo "111111";
		return send_valid_code(strtolower($checkcodes[0]), $mobile);
}
