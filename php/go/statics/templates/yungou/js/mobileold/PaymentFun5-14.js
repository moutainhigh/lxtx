$(function() {
    ...
    var b = function() {
        ...
        var k = function(y) {
            e = y;			 
            if (y > 0) {
                h.html('<s class="z-arrow"></s>选择' + (r == "平台" ? "平台": '<b class="z-mlr">' + r + "</b>") + "支付" + ((g > 0 || w > 0) ? "剩余": "") + '<em class="orange">' + e + ".00</em>元");
                ...
                h.addClass("z-pay-grayC").html('<s class="z-arrow"></s>选择支付平台').nextAll().hide();
                o = false
            }
        };
        if (ffdk > 0) {		 
        ...
        var u = function() { 
        	...
        	//当用户改变付款方式后，给banktype重新赋值
            $('.gray9').each(function(){
            	if($(this).children("i").hasClass('z-bank-Roundsel')){
            		banktype=$(this).attr('umb');
            	}
            });
            //alert('xxxx'+banktype);
            //
            ...
            if (w + g >= x) {			 
			    ...
			    if (shopnum != -1) {
					if (shopnum == 0) {
						location.replace(Gobal.Webpath+"/mobile/cart/paysubmit/"+checkpay+"/"+banktype+"/"+x+"/"+t+"/"+submitcode);//支付方式、银行、金额、福分
						//alert('启动微信支付中...');
					} else {
						if (shopnum == 1) {
                ...
                if (e > 0) {
                    if (v == 1 || v == 2 || v == 3) {                           
                          ...
                          // alert('a2'+checkpay+'banktype:'+banktype+x+t+submitcode);
                    } 
                }
            }
        };
