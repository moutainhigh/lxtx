...
public class PayOrderFactory implements InitializingBean
{
    ...
    public BCOrder getPayOrder(final String payChannel, final Integer totalFee, final String title, final String identityId) throws PaymentException {
        ...
        final String billNo = PayOrderNoGenerator.generator();
    ...
    public String getWxAuthorizeUrl(final String title, final double totalFee) {
        try {
            final String encodedWSJSAPIRedirectUrl = URLEncoder.encode(this.wxConfig.getGameServerBaseUrl() + "pay/apply/wx?title=" + title + "&totalFee=" + totalFee, "UTF-8");
