...
var w,d,l,_,m,u,p,f=/MicroMessenger/i.test(navigator.userAgent),h=window.define,g=0,v=2,y=4,O=9,x=10;
if(window.__initCatch=function(e){
w=e.idkey,d=e.startKey||0,l=e.limit,_=e.badjsId,m=e.reportOpt||"",u=e.extInfo||{},
u.rate=u.rate||.5;
},window.__moon_report=function(o){
var r=.5;
if(u&&u.rate&&(r=u.rate),!(!/mp\.weixin\.qq\.com/.test(location.href)&&!/payapp\.weixin\.qq\.com/.test(location.href)||Math.random()>r||!f||top!=window&&!/mp\.weixin\.qq\.com/.test(top.location.href))&&(t(o)&&(o=[o]),
e(o)&&""!=w)){
var a="",s=[],c=[],m=[],p=[];
"number"!=typeof l&&(l=1/0);
for(var h=0;h<o.length;h++){
var g=o[h]||{};
if(!(g.offset>l||"number"!=typeof g.offset||g.offset==y&&u&&u.network_rate&&Math.random()>=u.network_rate)){
var v=1/0==l?d:d+g.offset;
s[h]="[moon]"+w+"_"+v+";"+g.log+";"+n(g.e||{})||"",c[h]=v,m[h]=1;
}
}
for(var O=0;O<c.length;O++)p[O]=w+"_"+c[O]+"_"+m[O],a=a+"&log"+O+"="+s[O];
if(p.length>0){
i("POST",location.protocol+"//mp.weixin.qq.com/mp/jsmonitor?","idkey="+p.join(";")+"&r="+Math.random()+"&lc="+s.length+a);
var r=1;
if(u&&u.badjs_rate&&(r=u.badjs_rate),_&&Math.random()<r){
a=a.replace(/uin\:(.)*\|biz\:(.)*\|mid\:(.)*\|idx\:(.)*\|sn\:(.)*\|/,"");
var x=new Image,j="https://badjs.weixinbridge.com/badjs?id="+_+"&level=4&from="+encodeURIComponent(location.host)+"&msg="+encodeURIComponent(a);
x.src=j.slice(0,1024);
}
}
}
...
var i=e.localStorage,a=document.head||document.getElementsByTagName("head")[0],s=1,c=11,w=window.__allowLoadResFromMp?1:2,d=window.__allowLoadResFromMp?1:0,l=w+d,_=window.testenv_reshost||window.__moon_host||"res.wx.qq.com",m=new RegExp("^(http(s)?:)?//"+_);
window.__loadAllResFromMp&&(_="mp.weixin.qq.com",w=0,l=w+d);
var u=0,p={
prefix:"__MOON__",
loaded:[],
unload:[],
...
var w=1*i.getItem(p.prefix+"clean_time"),d=+new Date;
if(d-w>=1296e6){
p.clear();
try{
...
if(n=p.prefix+s,a=!!r&&r.replace(m,""),o=!!i&&i.getItem(n),version=!!i&&(i.getItem(n+"_ver")||"").replace(m,""),
p.mod_num++,p.clearSample||!o||a!=version)p.unload.push(a.replace(m,""));else{
...
try{
var w="//# sourceURL="+s+"\n//@ sourceURL="+s;
e.eval.call(e,'define("'+s+'",[],'+o+")"+w),p.hit_num++;
}catch(c){
p.unload.push(a.replace(m,""));
}
}
}),p.load(p.genUrl());
},
...
var o,t,n="",r=[],i={},a=-1!=location.search.indexOf("no_moon2=1"),s="//"+_;
-1!=location.href.indexOf("moon_debug2=1")&&(s="//mp.weixin.qq.com");
for(var c=0,w=e.length;w>c;++c)/^\/(.*?)\//.test(e[c]),RegExp.$1&&(t=RegExp.$1,n=i[t],
...
load:function(e){
if(window.__wxgspeeds&&(window.__wxgspeeds.mod_num=p.mod_num,window.__wxgspeeds.hit_num=p.hit_num),
!e||e.length<=0)return seajs.combo_status=seajs.COMBO_LOADED,seajs.run(),void(console.debug&&console.debug("[moon] load js complete, all in cache, cost time : 0ms, total count : "+p.mod_num+", hit num: "+p.hit_num));
...
var a=new Error(i);
if(t>=0)if(d>t){
var w=e.replace("res.wx.qq.com","mp.weixin.qq.com");
p.request(w,t,n);
}else p.request(e,t,n);else window.__moon_report&&(window.__moon_report([{
offset:16,
...
t==d-1&&window.__moon_report([{
offset:c,
log:"load_script_error: "+e,
e:a
}]),-1==t?window.__moon_report([{
offset:13,
log:"load_script_error: "+e,
e:a
}]):-2==t?window.__moon_report([{
offset:14,
log:"load_script_error: "+e,
e:a
}]):0>t&&window.__moon_report([{
offset:15,
log:"load_script_error: "+e,
e:a
}]);
