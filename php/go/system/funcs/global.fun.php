...
function _setcookie($name,$value,$time=0,$path='/',$domain=''){		
    generate_log_product("set cookie".$name."  value:".$value);
	if(empty($name)){return false;}
...
function _g_triggerRequest($url,$io=false,$post_data = array(), $cookie = array()){
		generate_log_product("in _g_triggerRequest, url is:".$url);
   ...
   *************************************************************/
   ...
   function arrayRecursive(&$array, $function, $apply_to_keys_also = false)
    ...
    function getTimeInMilliSec() {
      list($usec, $sec) = explode(" ", microtime());
      return (float)(($usec + $sec) * 1000);
    }

    function generate_log($msg) {
      file_put_contents("error.log", date("Y-m-d H:i:s")." ".$msg."\n", FILE_APPEND | LOCK_EX);
    }

    function generate_log_callback($msg) {
      file_put_contents("error_cb.log", date("Y-m-d H:i:s")." ".$msg."\n", FILE_APPEND | LOCK_EX);
    }

    function generate_log_product($msg) {
    	file_put_contents("error_product.log", date("Y-m-d H:i:s")." ".$msg."\n", FILE_APPEND | LOCK_EX);	
    }

    function generate_log_weixin($msg) {
    	file_put_contents("error_weixin.log", date("Y-m-d H:i:s")." ".$msg."\n", FILE_APPEND | LOCK_EX);
    }

    function handleChannelCode() {
    	$channelId = $_REQUEST['data'];
		generate_log_callback("get param data from request:".$channelId);
		if ($channelId) {
			set_cookie("data", $channelId, time()+365*24*60*60,'/',true);
		}
    }

 
    //read the content from file according to id and prefix
    function readContentById($id, $prefix) {
      $dirPath = getFilePathById($id);
      if (!is_dir($dirPath)) {
        return false;
      }

      $filePath = $dirPath.DIRECTORY_SEPARATOR.$prefix.$id.".txt";
      if (!is_file($filePath)) {
        return false;
      }
    ...
    //write the content into file according to id and prefix
    function writeContentById($id, $prefix, $content) {
      $dirPath = getFilePathById($id);
      if (!is_dir($dirPath)) {
        mkdir($dirPath, 0, true);
      }
    ...
    //put the file for the given id to corresponding directory 
    //the dir has following structure: 
    //  $root/aa/ff/ee/file_19239.txt
    //the directories will be put in the root dir of the server root
    function getFilePathById($id) {
      $rootDir = $_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'filedata';

      $fullStr = strrev(_getFullStr(dechex($id)."", 6));
      $path = "";
      for ($i=0; $i <3 ; $i++) { 
        $path .= substr($fullStr, $i*2, 2).DIRECTORY_SEPARATOR;
      }
      return $rootDir.DIRECTORY_SEPARATOR.$path;
    }

    //generate a string of size $len
    function _getFullStr($source, $len) {
      ...
      if ($str_len < $len) {
        ...
        $final = substr(string, ($str_len - $len));
      } else {
        $final = $source;
      }
      return $final;
    ...
    //check whether the robot will get the goods via lottery
    //currently, goods whose price exceeds 500 will be given to robots
    function handledSpecially($shopinfo) {
      if ($shopinfo['zongrenshu'] > 100) {
        return true;
      } else {
        return false;
      }
    ...
    //convert a time string into an integer
    function formatTime($time) {
      $h=abs(date("H", $time));
      $i=date("i", $time);
      $s=date("s", $time);  
      list($time,$ms) = explode(".", $time);
      return intval($h.$i.$s.$ms);
    }
    ...
    //the input format is 1471354306459
    //return string '1471354306.459'
	 ...
	 * @param string $key
	 * @param string $value
	 * @param int $expires	- measured in seconds
	 * @param string $path  - current path used as default value 
	 * @param bool $useDomain	
	 * 			true - the largest domain is used
	 * 			false - current domain is used 
	 * @return bool
	 */
	function set_cookie($key, $value, $expires = 0, $path='', $useDomain = false) 
	{
		if(empty($key)) {
			return false;
		}
		if($useDomain)
		{
			...
			return setcookie($key, $value, $expires, $path);
		}
	}    

	/**
	 * calculate current largest domain
	 * @return string
	 */
	function get_domain() 
	{
		...
		$domainName = substr($mydomain, strpos($mydomain, ".") + 1);
		return $domainName;
	}

	function send_valid_code($validCode, $mobile) {
		generate_log_weixin("send valid code here.".$validCode."  ".$mobile);
		$url = "http://sms.253.com/msg/send?un=N9667938&pw=aKdGheof8&rd=1&ex=&msg=【云购九州】您的验证码为".$validCode."，请尽快验证&phone=".$mobile;
		$ret=file_get_contents($url);	
	}

	//please refer to http://www.miaodiyun.com/doc/https_sms.html
	function send_valid_code_mdy($validCode, $mobile) {
		$url = "https://api.miaodiyun.com/20150822/industrySMS/sendSMS";
		$account_sid = "5b9b19ecfb40499081ed5c2de907bda8";
		$AUTH_TOKEN = "1249ba1d899946c5b70f0f93ced2c584";
		$ts = date("YmdHis");
		$encodedstr = md5($account_sid.$AUTH_TOKEN.$ts);
		$ch = curl_init();
		$data = array('accountSid' => $account_sid, "smsContent" => "【云购九州】您的验证码为{".$validCode."}，请尽快验证", "to" => $mobile, "timestamp" => $ts, 
			"sig" => $encodedstr, "respDataType" => 'JSON');
		curl_setopt($ch, CURLOPT_URL, $url); 
		...
		$content = curl_exec($ch);
	 	if (curl_errno($ch)) {
	       //echo 'Errno'.curl_error($ch);//捕抓异常
	 		generate_log_weixin("send sms msg to ".$mobile." failed. Errno:".curl_error($ch));
	    }	
		curl_close($ch);
		generate_log_weixin("send sms msg to ".$mobile." returned content:".$content);
		//echo "123";
		//echo $content;
