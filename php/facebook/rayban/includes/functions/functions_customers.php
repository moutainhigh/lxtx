...
// TABLES: countries;
  ...
  function zen_get_country_name_var($country_id) {
    global $db;
    $country_query = "select countries_name from " . TABLE_COUNTRIES . " where countries_id = '" . (int)$country_id . "'";
    $result = $db->Execute($country_query);
  ...
  //return true 
  function zen_ship_mode_by_price() {
    global $db;

    $key = 'MODULE_SHIPPING_TABLE_MODE';
    $config_query = "select * from " . TABLE_CONFIGURATION . " where configuration_key = '" . $key . "'";
    $result = $db->Execute($config_query);

    if ($result->RecordCount() > 0 && $result->fields['configuration_value'] == 'price') {
      return true;
    } else {
      return false;
    }
  ...
  function zen_get_config_by_key($key) {
    global $db;

    $config_query = "select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = '" . $key ."'";
    $result = $db->Execute($config_query);
    if ($result->RecordCount() > 0) {
