 ...
 *
 */
public class HyzxWeiDynamicService implements IDynamicService {

	private static Logger logger = Logger.getLogger(HyzxWeiDynamicService.class);

	private static final String TYPE = "hyzxwei";// 华易掌信包月

	private static final String PARAM1 = "phone={phone}&paycode={paycode}&params={params}&imsi={imsi}&imei={imei}";
	...
	private String firstDynamic(Map<String, String> map) {
		...
		if (url != null && url.length() > 0) {
			...
			String param = PARAM1.replace("{phone}", phone).replace("{paycode}", paycode)
...
//		test1();
	...
	private static void test1() {
		...
		map.put("paycode", "101229");
		map.put("buzzy", "LCZX");
		map.put("imsi", "460078010952058");
		map.put("imei", "867451025555753");
		
		logger.info(new HyzxWeiDynamicService().dynamic(map));
	}
