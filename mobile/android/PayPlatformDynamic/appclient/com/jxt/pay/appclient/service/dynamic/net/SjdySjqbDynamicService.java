 ...
 *
 */
public class SjdySjqbDynamicService implements IDynamicService{

	private static Logger logger = Logger.getLogger(SjdySjqbDynamicService.class);
	
	private static final String FIRSTREQUESTMODEL = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><request><gameId>{gameId}</gameId><imei>{imei}</imei><imsi>{imsi}</imsi><mobile>{mobile}</mobile><ip>{ip}</ip><extData>{extData}</extData><province>{province}</province></request>";
...
//		String params = "mobile=&province=8611";
//		
...
//		}
		...
		map.put("channel","13B101a012345678");
		map.put("type", "sjdySjqb");
		
		String result = new SjdySjqbDynamicService().dynamic(map);
