      ...
      switch ($column_list[$col]) {
        ...
        case 'PRODUCT_LIST_NAME':
          ...
          if (isset($_GET['manufacturers_id'])) {
            $lc_text = '<h2 class="product-name"><a href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'products_id=' . $listing->fields['products_id']) . '">' . zen_trunc_string($listing->fields['products_name'], 90, '...') . '</a></h2><div class="listingDescription">' . zen_trunc_string(zen_clean_html(stripslashes(zen_get_products_description($listing->fields['products_id'], $_SESSION['languages_id']))), PRODUCT_LIST_DESCRIPTION) . '</div>' ;
          } else {
            $lc_text = '<h2 class="product-name"><a href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), ($_GET['cPath'] > 0 ? 'cPath=' . $_GET['cPath'] . '&' : '') . 'products_id=' . $listing->fields['products_id']) . '">' . zen_trunc_string($listing->fields['products_name'], 90, '...') . '</a></h2><div class="listingDescription">' . zen_trunc_string(zen_clean_html(stripslashes(zen_get_products_description($listing->fields['products_id'], $_SESSION['languages_id']))), PRODUCT_LIST_DESCRIPTION) . '</div>';
          }
          break;
        case 'PRODUCT_LIST_MANUFACTURER':
          $lc_align = '';
        ...
        case 'PRODUCT_LIST_PRICE':
          ...
          $lc_align = 'right';
          $lc_text =  '<div class="price-box">' . $lc_price . '</div>';
...
// qty box with add to cart button
        ...
        if ($listing->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) {
          ...
          if (isset($_GET['manufacturers_id'])) {
          $lc_text = '<a  class="product-image" href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'products_id=' . $listing->fields['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $listing->fields['products_image'], $listing->fields['products_name'], IMAGE_PRODUCT_LISTING_WIDTH, IMAGE_PRODUCT_LISTING_HEIGHT, 'class="listingProductImage"') . '</div></a>';
          } else {
          $lc_text = '<a class="product-image"  href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), ($_GET['cPath'] > 0 ? 'cPath=' . $_GET['cPath'] . '&' : '') . 'products_id=' . $listing->fields['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $listing->fields['products_image'], $listing->fields['products_name'], IMAGE_PRODUCT_LISTING_WIDTH, IMAGE_PRODUCT_LISTING_HEIGHT, 'class="listingProductImage"') . '</a>';
          }
        }
          break;
      }
    ...
    if (PRODUCT_LISTING_LAYOUT_STYLE == 'columns') {
	  if($rows == 0){ $row_class = ' first_row'; } else { $row_class = ''; }
	  if($column == 0){ $col_class = ' first'; } elseif($column == PRODUCT_LISTING_COLUMNS_PER_ROW - 1) { $col_class = ' last'; } else { $col_class = ''; }
      $lc_text = implode('', $product_contents);
      $list_box_contents[$rows][$column] = array('params' => 'class="ware fl'.$row_class.$col_class.'"',
                                                 'text'  => $lc_text);
      $column ++;
