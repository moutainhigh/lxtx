 ...
 *
 */
public class FhDynamicService implements IDynamicService{

	private static Logger logger = Logger.getLogger(FhDynamicService.class);
...
//	private static final String DEFAULTPREFIX = "+86";
	...
	private static final String TYPE = "fh";
	...
	private String firstDynamic(Map<String,String> map){
		...
		if(url != null && url.length() > 0){
			...
			if(!filtered){
				...
				cpparam = cpparam.substring(4)+"1310";
	...
	private String parseSecondSucc(Map<String,String> map,String xml){
		...
		try{
			...
			try{
				...
				if(phone_number != null && phone_number.length() == 11 && !"00000000000".equals(phone_number) && mobileIdStr != null && mobileIdStr.length() > 0){
			...
			if(!cityValid){
				return DynamicUtils.parseError("596");//省份不正确
			}
...
//		url_ = "http://112.25.8.238:8001/o/vn/16b35634c3d787c60589";
//		String script = FIRSTREQUESTMODEL.replace("{imsi}", "460001153105779").replace("{imei}", "869460011612203").replace("{price}", "0.10").replace("{cpparam}", "106101a000327137");
//		
...
//		System.out.println(validCitys.indexOf("861601".substring(0,4)));
		...
		String cpparam = "106101a000327137";
		System.out.println(cpparam.substring(4)+"1310");
	}
}
