...
class auto_register extends admin {
	...
	foreach ($res as $k => $v) {
		...
		$img = isset($v[4])?$v[4]:'photo/member.jpg';//头像
		if(!$password){
			$password =md5('111111');
		}else{
		...
		if(is_array($member_e)){
			if(!is_array($member_m)){
				$this->db->Query("INSERT INTO `@#_member`(username,password,mobile,img,emailcode,mobilecode,time,auto_user)VALUES('$username','$password','$mobile','$img','-1','1','$time','1')");
				$tems++;
			}
		}else{
			if(is_array($member_m)){
				$this->db->Query("INSERT INTO `@#_member`(username,password,email,img,emailcode,mobilecode,time,auto_user)VALUES('$username','$password','$email','$img','1','-1','$time','1')");
			}else{
				$this->db->Query("INSERT INTO `@#_member`(username,password,email,mobile,img,emailcode,mobilecode,time,auto_user) VALUES ('$username','$password','$email','$mobile','$img','1','1','$time','1')");
			}
