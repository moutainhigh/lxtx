...
public class CarServlet extends org.apache.catalina.websocket.WebSocketServlet {
    ...
    protected StreamInbound createWebSocketInbound(String subProtocol,HttpServletRequest request) {
        return new WebSocketMessageInbound(NetConstants.CAR_CHIP_QUEUE, maxClientId.incrementAndGet());
    }
	...
	public void init() throws ServletException {
		...
		new Thread(new BotOperQueueListener(NetConstants.CAR_CHIP_QUEUE)).start();
		WebSocketMessageInboundPool pool = WebSocketInboundPoolFactory.getWebSocketMessageInboundPool(NetConstants.CAR_CHIP_QUEUE);
		new Thread(new MainProcQueueListener(NetConstants.CAR_MAIN_SEQ_QUEUE_IN, pool)).start();
	}
}