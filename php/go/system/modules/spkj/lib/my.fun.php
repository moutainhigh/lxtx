...
function shopimg($shopid){
	...
	$shop=$mysql_model->GetOne("select * from `@#_shoplist_shipin` where `id`='$shopid'");
	return $shop['thumb'];
}
function shoplisext($id,$zd){
	$mysql_model=System::load_sys_class('model');
	//$shop=$mysql_model->GetOne("select * from `@#_shoplist` where `id`='$id'");
	$shop=$mysql_model->GetOne("select * from `@#_shoplist_shipin` where `id`='$id'");
	return $shop[$zd];
...
function yunci($uid,$shopid,$shopqishu){
	...
	$record2=$mysql_model->GetList("select * from `@#_member_go_record_shipin` where `uid`='".$uid."' and `shopid`='".$shopid."' and `shopqishu`='".$shopqishu."'");
	$cord="";
...
function yunmas($ma,$html="span",$limit="9999"){
	$list=explode(",",$ma);
	$st="";
	foreach($list as $i => $list2){
		if($i>=$limit)
		{
			break;
		}
...
function get_shop_if_jiexiao($shopid=null){
	...
	$record=$db->GetOne("select * from `@#_shoplist_shipin` where `id`='$shopid' and `q_showtime` = 'N' LIMIT 1");
...
function huodez($shopid,$shopqishu,$canshu=null){
	...
	$record=$mysql_model->GetOne("select * from `@#_member_go_record_shipin` where `shopid`='$shopid' and `shopqishu`='$shopqishu' and `huode`>'10000000'");
