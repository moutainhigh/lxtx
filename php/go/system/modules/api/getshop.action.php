...
class getshop extends SystemAction {
	...
	public function lottery_shop_json(){
		...
		if($q_time <= time()){
			$db->Query("update `@#_goods_item` SET `q_showtime` = 'N' where `id` = '$info[id]' and `q_showtime` = 'Y' and `q_uid` is not null");
			echo json_encode(array("error"=>'-1'));			
			return;	exit;
		}	
	...
	public function lottery_shop_set(){
		if(isset($_POST['lottery_sub'])){
			...
			$q = false;
			if(time() >= $q_time){
				$q = $db->Query("update `@#_goods_item` SET `q_showtime` = 'N' where `id` = '$gid' and `q_showtime` = 'Y' and `q_uid` is not null");	
			}
			if($q){
