...
// eof: adjust new quantity to be same as current in stock
  ...
  function calculate() {
    ...
    $contents_str = json_encode($this->contents);
    $this->generate_log("content is : " . $contents_str);
...
//echo 'products_id: ' . $product->fields['products_id'] . ' Prices ' . '$chk_price: ' . $chk_price . ' $chk_special: ' . $chk_special . ' attributes_price_factor:' . $attribute_price->fields['attributes_price_factor'] . ' attributes_price_factor_offset: ' . $attribute_price->fields['attributes_price_factor_offset'] . '<br>';
      ...
      $this->generate_log("product total is: " . $productTotal);
      // attributes weight
      if (isset($this->contents[$products_id]['attributes'])) {
        reset($this->contents[$products_id]['attributes']);
        while (list($option, $value) = each($this->contents[$products_id]['attributes'])) {
...
//echo 'shopping_cart class Price: ' . $productTotal . ' qty: ' . $qty . '<br>';
      $this->generate_log(" total0 is : " . $productTotal." ".$products_tax." ".$qty);
      $priceA = zen_round(zen_add_tax($productTotal, $products_tax), $decimalPlaces);
      $this->total +=  $priceA * $qty;
      $this->contents[$products_id]['price_pre'] = $priceA;

      $this->generate_log(" total1 is : " . $this->total);      
      $this->total += zen_round(zen_add_tax($totalOnetimeCharge, $products_tax), $decimalPlaces);
      $this->generate_log(" total2 is : " . $this->total);      
      //now the value of $total is before discount
      //we save the price and quantity of the product here
...
//echo 'Product Attribute before: ' . $new_attributes_price_before_discounts . '<br>';
    ...
    $this->generate_log("contents is :".json_encode($this->contents));
    //now calculate the final price
    $top_product_id = 0;
    $top_product_price = 0;

    foreach ($this->contents as $pid => $val) {
    ...
    $this->generate_log("top price is :" . $top_product_price. " top product is:".$top_product_id);
    $this->total_pre = $this->total;
    $this->total = 0;
    foreach ($this->contents as $pid => $val) {
      $p_price = $val['price_pre'];
      if ($pid == $top_product_id) {
        $this->total +=  $p_price + round($p_price * DISCOUNT_RATE * ($val['qty'] - 1), 2);
      } else {
        $this->total +=  round($p_price * DISCOUNT_RATE * $val['qty'], 2);
      }
    }
    //$this->final_price = $final_price;
    $this->generate_log("now the final price is:".$this->total);
  }
...
//echo 'what is it: ' . ' : ' . $products_id . ' - ' . $value . ' records: ' . $chk_attributes_exist->RecordCount() . ' vs ' . print_r($this->contents[$products_id]) . '<br>';
    ...
    $this->fillProductsArray($products_array);
    $this->generate_log(json_encode($products_array));
    $this->sortArrByOneField($products_array, "final_price", true);
...
function sortArrByOneField(&$array, $field, $desc = false){
  ...
  function fillProductsArray(&$products_array) {
    global $currencies;

    $top_product_id = 0;
    $top_product_price = 0;

    foreach ($products_array as $idx => $product) {
      if ($product['final_price'] > $top_product_price) {
        $top_product_id = $product['id'];
        $top_product_price = $product['final_price'];
      }
    }

    //$this->generate_log("top price is :" . $top_product_price. " top product is:".$top_product_id);
    $total_pre = $this->total;
    $total = 0;
    $productInfo = array();
    
    //$this->generate_log(json_encode($this->contents));
    foreach ($this->contents as $pid => $val) {
      $p_price = $val['price_pre']; //to fix the .50
      $price_discout = round($p_price * DISCOUNT_RATE, 2);
      if ($pid == $top_product_id) {
         $extra_num = $val['qty'] - 1;
         $price_discout = round($p_price * DISCOUNT_RATE, 2);
         $total +=  $p_price + round($price_discout * $extra_num, 2); //round($p_price * DISCOUNT_RATE * ($extra_num), 2);
         $extra_desc = $extra_num > 0 ? $currencies->format($price_discout)." *".$extra_num : "";
         $v1 = $currencies->format(12.99);
         $final_price_desc = $currencies->format($p_price)."*1"." ".$extra_desc;
      } else {
         $total +=  round($price_discout * $val['qty'], 2);
         $final_price_desc = $currencies->format($price_discout)."*".$val['qty'];
      }
      $productInfo[$pid] = array("final_total_price" => $total, "final_price_desc" => $final_price_desc);
      $total = 0;
    }
    ...
    $idx = 0;
    foreach ($products_array as $idx => $product) {
      $p = $productInfo[$product['id']];
      $product['final_price_desc'] = $p['final_price_desc'];
      $product['final_total_price'] = $currencies->format($p['final_total_price']);
      //$this->generate_log(json_encode($product));
      $arr[$idx++] = $product;
    }
    //$this->generate_log(json_encode($products_array));
    $products_array = $arr;
  ...
  function show_total_pre() {
    //return $this->total_fi
