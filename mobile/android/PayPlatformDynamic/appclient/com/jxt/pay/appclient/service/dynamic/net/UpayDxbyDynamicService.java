 ...
 *
 */
public class UpayDxbyDynamicService implements IDynamicService{

	private static Logger logger = Logger.getLogger(UpayDxbyDynamicService.class);
	
	private static final String TYPE = "upayDxby";
	
	private static final String PARAM1 = "paykey={paykey}&imsi={imsi}&cp_param={cp_param}";
	...
	public String dynamic(Map<String, String> map) {
		...
		if(url != null && url.length() > 0){
			...
			String param = PARAM1.replace("{paykey}", paykey).replace("{imsi}",imsi).replace("{cp_param}",cp_param);
...
//		test3();
	...
	private static void test1(){
		...
		map.put("type",TYPE);
		map.put("theNo","1");
		map.put("imsi","460023501847588");
		map.put("paykey","306600020000000-10677375-618373246");
		map.put("cp_param","LX13A301");
		
		logger.info(new UpayDxbyDynamicService().dynamic(map));
	}
