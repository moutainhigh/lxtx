...
import org.takeback.chat.controller.UserController;
...
public class GameMonitor
{
	...
	private static Logger logger = LoggerFactory.getLogger(GameMonitor.class);
    ...
    public void setData(final String roomId, final Integer uid, final Double inoutNum) {
        try {
            ...
            if (c == null) {
                c = new ControlModel(roomId, r.getName(), uid, u.getUserId(), u.getNickName(), NumberUtil.round(Double.valueOf(SystemConfigService.getValue("control_default_rate")) / 100.0));
                this.cache.add(c);
            }
            ...
            if ("1".equals(SystemConfigService.getValue("control_flag")) && c.getPlayTimes() >= GameMonitor.START_CONTROL) {
                this.doControl(c);
            }
        }
        catch (Exception e) {
    ...
    private void doControl(final ControlModel c) {
        ...
        final Double winControlRate = Double.valueOf(SystemConfigService.getValue("control_kill")) / 100.0;
        final Double winForceControlRate = winControlRate + 0.1;
        final Double loseControlRate = -Double.valueOf(SystemConfigService.getValue("control_save")) / 100.0;
    ...
    private Double getPointMaybeWin(final Room r) {
        if (r.getType().startsWith("G01")) { //if the value is larger than the average value, it will win
    ...
    public BigDecimal getOne(final String roomId, final Integer uid) {
        try {
            ...
            if (values != null && values.length() > 0) {
                ...
                final Double d = Double.valueOf(v[0]);
     ...
     * @return
     */
	public Integer adjustCoin(Integer getCoin, int senderId, int uid, String roomId, String lotteryDesc) {
		Room room = this.roomStore.get(roomId);
		if (room.getType().startsWith("G04")) { //mine
			User sender = this.userStore.get(senderId);
			User player = this.userStore.get(uid);
			if (sender.getUserType().equals("9") && player.getUserType().equals("2")) { //robot
				//let the player hit the bomb
				logger.info("adjust the number");

				String human_mine_rate = SystemConfigService.getValue("human_mine_rate");
				
				if (Strings.isNullOrEmpty(human_mine_rate)) {
					return getCoin;
				} else {
					int rate = Integer.valueOf(human_mine_rate).intValue();
					Random r = new Random();
					if (r.nextInt(rate) == 1) {
						return ((int)(getCoin/10))*10 + GameMonitor.getBombNumber(lotteryDesc);
					} else {
						return getCoin;
					}					
				}
			...
			//for other games, directly return the original value
			return getCoin;
		}
	}
	
	private static int getBombNumber(String desc) {
		String ch = desc.trim().substring(desc.length() - 1);
