$(function() {	
    var a = function() {	 
        ...
        var r = function() {
            ...
            var y = function() {
               ...
               // GetJPData("http://m.1yyg.com", "getUserRecharge", w(),
				 GetJPData(Gobal.Webpath, "shopajax", "getUserRecharges"+w(),
                function(D) {
                    if (D.code == 0) {
                        var B = "";
                        if (v.isCount == 1) {
                            x = D.count;
                            v.isCount = 0;
                            B = '<li class="m-userMoneylst-tt"><span>消费时间</span><span>消费金额</span><span>消费内容</span></li>'
                        }
                        var C = D.listItems;
                        var A = C.length;
                        for (var z = 0; z < A; z++) {
        ...
        var t = function() {
            ...
            var y = function() {
                d.show();				
                GetJPData(Gobal.Webpath, "shopajax", "getUserConsumptions"+w(),
                function(D) {
                    if (D.code == 0) {
                        var B = "";
                        if (v.isCount == 1) {
                            v.isCount = 0;
                            x = D.count;							 
                            B = '<li class="m-userMoneylst-tt"><span>获得时间</span><span>获得金额</span><span>获得渠道</span></li>'
                        }
                        var C = D.listItems;
                        var A = C.length;
                        for (var z = 0; z < A; z++) {
