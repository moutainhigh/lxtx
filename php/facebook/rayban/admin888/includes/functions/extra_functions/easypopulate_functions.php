...
function ep_get_tax_class_rate($tax_class_id) {
	global $db;	
	$tax_multiplier = 0;
	$tax_query = mysqli_query($db->link, "select SUM(tax_rate) as tax_rate from " . TABLE_TAX_RATES . " WHERE  tax_class_id = '" . zen_db_input($tax_class_id) . "' GROUP BY tax_priority");
	if (mysqli_num_rows($tax_query)) {
		while ($tax = mysqli_fetch_array($tax_query)) {
...
function ep_get_tax_title_class_id($tax_class_title) {
	global $db;	
	$classes_query = mysqli_query($db->link, "select tax_class_id from " . TABLE_TAX_CLASS . " WHERE tax_class_title = '" . zen_db_input($tax_class_title) . "'" );
	$tax_class_array = mysqli_fetch_array($classes_query);
  ...
  $products = $db->Execute($sql);
	...
	if (mysqli_errno($db->link)) {
		...
		if ($ep_debug_logging == true) {
			...
			$string = "MySQL error ".mysqli_errno($db->link).": ".mysqli_error($db->link)."\nWhen executing:\n$sql\n";
...
function install_easypopulate() {
	...
	$db->Execute("INSERT INTO admin_pages VALUES ('easyPopulateConfig', 'BOX_CONFIGURATION_EASY_POPULATE', 'FILENAME_CONFIGURATION', 'gID=".$group_id."', 'configuration', 'Y', '26')");
...
function remove_easypopulate() {
	...
	if (mysqli_num_rows($result)) {
		...
		$ep_groups =  mysqli_fetch_array($result);
