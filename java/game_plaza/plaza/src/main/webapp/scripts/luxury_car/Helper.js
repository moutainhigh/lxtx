...
function showHelper(){
	...
	layer_helper_background.x = converToParentX(640, baseLayer.getWidth());
	layer_helper_background.graphics.drawRect(1, "#223850", [0, 0, 640, 585], true, "#223850");
	...
	layer_bank_title.graphics.drawRect(1, "#162737", [0, 0, 640, 90], true, "#162737");
	...
	if(window.innerWidth < window.innerHeight){
		layer_webView.display.style.setProperty("transform", "rotate(-90deg)");
		var p = layer_split.localToGlobal(new LPoint(layer_split.x,layer_split.y))
		layer_webView.setViewPort(new LRectangle(50, p.y - 80,width,400));
	}else{
		layer_webView.display.style.setProperty("transform", "rotate(0deg)");
		layer_webView.setViewPort(new LRectangle(converToParentX(640, baseLayer.getWidth()) ,100,width,450));
	}
	layer_webView.loadURL(global_util.getAppPath()+"/luxurycar_helper.html");
	layer_webView.show();
