...
<body>
   ...
   <div class="pasts">
       ...
       <div class="dootm"><p>没有更多啦，当天揭晓结果已加载完毕</p></div>
   </div>
   ...
   <script>
   	$(document).ready(function() {
   		...
   		$.get(path + "/lotteryorder/cqsscTodayDatas", {}, function(data) {
			var template = "{{#recordList}}<div class=\"li\"><div class=\"lista\"><span>第{{serialNumber}}期</span></div><div class=\"listb\">{{#codeList}}<p><span>{{.}}</span>{{/codeList}}</p></div><div class=\"listc\"><i></i><span>{{openTimeStr}}</span></div></div>{{/recordList}}";
			var container = $(".pasts").eq(0).find(".ul").eq(0);
