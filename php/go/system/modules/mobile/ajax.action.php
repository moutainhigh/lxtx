   ...
   // 今日揭晓商品
	...
	if($ShopId==0 || $ShopNum==0){
	  ...
	  $cart['code']=1;   //表示添加失败
	   ...
	   generate_log_product("set Cartlist:".json_encode($Mcartlist));
...
public function delCartItem_jf(){
	...
	public function userMobile(){
		...
		$sql="INSERT INTO `@#_member`(`mobile`,password,img,emailcode,mobilecode,yaoqing,reg_key,time)VALUES('$name','$pass','photo/member.jpg','-1','-1','$decode', '$data','$time')";
		...
		if(!$name || $this->db->Query($sql)){
			...
			generate_log_callback("reg success");
			$userMobile['state']=0;

		}else{

			//_message("注册失败！");
			generate_log_callback("reg fail");
	...
	public function getLotteryList(){
	   ...
	   $shopsum=$this->db->GetOne("SELECT count(*) AS total FROM `@#_shoplist` WHERE `q_uid` >= 1 AND `hot` <> 2 AND `q_showtime` = 'N'");



	   //最新揭晓

		$shoplist['listItems']=$this->db->GetList("SELECT id, thumb, q_uid, q_user_code, q_end_time FROM `@#_shoplist` WHERE `q_status` = 2 AND `hot` <> 2 AND `q_showtime` = 'N' ORDER BY `q_end_time` DESC limit $FIdx,$EIdx");
	...
	public function getUserBuyList(){
	   ...
	   $fieldList = "thumb, id, qishu, money, q_user_code, q_end_time, title";
	   $fieldListB = "b.thumb, b.id, b.qishu, b.money, b.q_user_code, b.q_end_time, b.title, b.qishu, b.canyurenshu, b.zongrenshu, b.shenyurenshu";
		 ...
		 if($type==0){
		  ...
		  $shoplist=$this->db->GetList("select $fieldListB, sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$uid' GROUP BY shopid ");



		  $shop['listItems']=$this->db->GetList("select $fieldListB, sum(gonumber) as gonumber from `@#_member_go_record` a left join `@#_shoplist` b on a.shopid=b.id where a.uid='$uid' GROUP BY shopid order by a.time desc limit $FIdx,$EIdx " );

		 }elseif($type==1){

		   //获得奖品

		    $shoplist=$this->db->GetList("select $fieldList from  `@#_shoplist`  where q_uid='$uid' " );



		    $shop['listItems']=$this->db->GetList("select $fieldList from  `@#_shoplist`  where q_uid='$uid' order by q_end_time desc limit $FIdx,$EIdx" );
	...
	public function GetStartRaffleAllList(){
		...
		$list = $this->db->getlist("select qishu,xsjx_time,id,thumb,title,q_uid,q_user,q_end_time,money from `@#_shoplist` where `hot` <> 2 and `q_showtime` = 'Y' AND id > '$maxSeconds' order by `q_end_time` DESC limit 0,10");
	...
	public function BarcodernoInfo(){
		...
		$res = $this->db->Query("UPDATE `@#_goods_item` SET `q_showtime`='N' where `id`= $itemid");
