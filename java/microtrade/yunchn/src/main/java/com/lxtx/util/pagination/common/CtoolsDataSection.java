...
public class CtoolsDataSection {
    ...
    private static final ThreadLocal<CtoolsException> _exception = new ThreadLocal<CtoolsException>();
    ...
    public static CtoolsException getException() {
        return _exception.get();
    }

    public static void setException(CtoolsException ex) {
        _exception.set(ex);
    ...
    public static Object get(ApiParam param) {
        ...
        if (param.isRequired()) {
            throw new CtoolsException(1000, "缺乏属性[" + mapping + "]的值");
        }
