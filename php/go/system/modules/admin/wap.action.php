...
class wap extends admin {
	...
	public function add(){
		...
		$this->db->Query("insert into `@#_wap`(`title`,`link`,`img`,`color`,`status`) values('$title','$link','$img','$title2','$status') ");	
	...
	public function update(){
		...
		if(isset($_POST['submit'])){
			...
			$this->db->Query("UPDATE `@#_wap` SET `status`='$status',`img`='$img',`title`='$title',`link`='$link',`color`='$title2' WHERE `id`=$id");
			if($this->db->affected_rows()){
