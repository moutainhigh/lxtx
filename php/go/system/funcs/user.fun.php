...
* key  获取完整用户名, sub 截取,all 完整
*/
function get_user_name_open($uid='',$type='username',$key='sub'){
	if(is_array($uid)){			
			if(isset($uid['username']) && !empty($uid['username'])){
				if($key=='sub'){
					$username_count=strlen($uid['username']);
					if($username_count>20){
						return $uid['username']=substr($uid['username'],0,9).'**';
					}else{
						return $uid['username'];
					}
				}else{
		...
		if(isset($info['username']) && !empty($info['username'])){	
				if($key=='sub'){
					$username_count=strlen($info['username']);
					if($username_count>20){
						return $info['username']=substr($info['username'],0,9).'**';
					}else{
						return $info['username'];
					}
				}else{
