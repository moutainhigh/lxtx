...
function dispatchMsg(strMsg){
	 ...
	 switch(msg.protocol){
		...
		case S2C_PROTOCOL_TICKOUT:
			is_tick_out = true;
			alert("账号在其他地方登陆，请确认账号安全！如有异常请联系官方微信公众号\"乐玩九州\"。");
			break;
	 }
}
function GetRequest() { 
...
function processGameState(msg){
	...
	//alert("state:"+curr_game_state+" interver:"+curr_game_interval);
...
function clearChipsLayer(){
	...
	var ok = false;
	for (var i = IDX_SX_SHU; i <= IDX_1V88; i++){
	...
	array_curr_chips_layer.splice(0, IDX_1V88);	
}
function clearLastSettedChips(){
	// array_last_setted_chips.splice(0, IDX_1V88);	
}
...
function doOpenResult(e){
	if (-1 != gAllChips.indexOf(curr_result.result_sx) || 
		-1 != gAllChips.indexOf(curr_result.result_wx) ||
		-1 != gAllChips.indexOf(curr_result.result_td)){
		gResultMusic = 1;
	}else{
		gResultMusic = 0;
	}
