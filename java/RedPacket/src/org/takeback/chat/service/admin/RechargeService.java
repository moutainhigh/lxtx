...
public class RechargeService extends MyListService
{
    ...
    public Map<String, Object> list(final Map<String, Object> req) {
        final String entityName = req.get(RechargeService.ENTITYNAME).toString();
        if (StringUtils.isEmpty((CharSequence)entityName)) {
            throw new CodedBaseRuntimeException(404, "missing entityName");
        }
        final int limit = Integer.parseInt(req.get(RechargeService.LIMIT).toString());
        final int page = Integer.parseInt(req.get(RechargeService.PAGE).toString());
        final Map<String, Object> myQuires = (Map<String, Object>) req.get("myQuires");
        final String orderInfo = req.get(RechargeService.ORDERINFO).toString();
        ...
        if (myQuires.containsKey("uid") && !"".equals(myQuires.get("uid"))) {
            queryUser = Integer.parseInt(myQuires.get("uid").toString());
        }
        String idlimit = null;
