 ...
 *
 */
public class SysjWoDynamicService implements IDynamicService{
	
	private static Logger logger = Logger.getLogger(SysjWoDynamicService.class);
	private static final String TYPE = "sysjWo";
	
	private static final String PARAMS = "mobile={mobile}&chargeCode={chargeCode}";
...
//		String responseJson = PostParamsData.postData(url,pairs);
		...
		String params = PARAMS.replace("{mobile}", mobile).replace("{chargeCode}", chargeCode);
		
		String responseJson = GetData.getData(url,params);
		
		try{
//			JSONObject joparam = new JSONObject();
//			joparam.put("mobile", mobile);
//			joparam.put("chargeCode",chargeCode);
//			
...
//		test2();
	...
	private static void test1(){
		...
		map.put("url","http://115.29.201.88/sync/hzdsworeq");
		map.put("type","sysjWo");
		map.put("mobile","15692182978");
		map.put("chargeCode","HY2203800800");
		map.put("theNo","1");
		
		System.out.println(new SysjWoDynamicService().dynamic(map));
	}
